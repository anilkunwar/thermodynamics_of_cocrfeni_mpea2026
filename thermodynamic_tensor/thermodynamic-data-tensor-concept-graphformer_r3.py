...
# -*- coding: utf-8 -*-
"""
Thermodynamic Data Tensor (Gibbs Tensor) Concept Graph v6.1
===========================================================
Multi‑level reasoning concept graph for numerical/quantitative description of
thermodynamic data tensors (Gibbs tensor) as a function of composition and temperature.

This is a TRUE architectural port of the MPEA‑ConceptGraph codebase, preserving
every memory‑safe pattern, visualisation pattern, and session‑state management pattern.

DOMAIN: Thermodynamic Data Tensor (Gibbs Tensor)
- Tensor fundamentals: tensor rank, Tucker/CP decomposition, tensor completion,
  core tensor, factor matrices, alternating least squares, low‑rank approximation,
  Kronecker product.
- Thermodynamic quantities: Gibbs free energy, excess Gibbs, enthalpy/entropy of mixing,
  chemical potential, activity coefficient, heat capacity, thermal expansion.
- CALPHAD methods: Redlich–Kister polynomials, sublattice model, interaction parameters,
  Scheil–Gulliver, Muggianu extrapolation, Gibbs–Duhem, phase diagrams.
- Phase‑field & numerical methods: phase‑field model, Allen–Cahn, Cahn–Hilliard, KKS,
  grand potential, order parameter.
- Data‑driven methods: PINNs, tensor neural networks, GPR, active learning, Bayesian
  optimisation, UQ.
- Multiscale & first‑principles: DFT, MD, cluster expansion, SQS.
...
Run:
    streamlit run thermodynamic_tensor_concept_graph.py
...
st.set_page_config(
    page_title="Thermodynamic Data Tensor (Gibbs Tensor) Concept Graph v6.1",
    page_icon="🔬",
    layout="wide",
...
# ============================================================================
# The rest of the application code follows (as provided in the snippet)
# ============================================================================
# (All classes, functions, and the main() definition are included below.)
...
# ----------------------------------------------------------------------------
# ENHANCED ONTOLOGY & NLP REASONING SYSTEM (THERMODYNAMIC DATA TENSOR)
# ----------------------------------------------------------------------------
...
# 32 Specialized Latent Experts for Thermodynamic Data Tensor
TENSOR_EXPERT_LABELS = [
    "Tensor: Rank", "Tensor: Tucker Decomp", "Tensor: CP Decomp",
    "Tensor: Completion", "Tensor: Core Tensor", "Tensor: Factor Matrix",
    "Tensor: ALS", "Tensor: Low-Rank", "Tensor: Kronecker",
    "Tensor: Contraction", "Thermo: Gibbs Energy", "Thermo: Excess Gibbs",
    "Thermo: Enthalpy Mix", "Thermo: Entropy Mix", "Thermo: Chemical Potential",
    "Thermo: Activity Coeff", "Thermo: Heat Capacity", "Thermo: Thermal Expansion",
    "CALPHAD: Redlich-Kister", "CALPHAD: Sublattice", "CALPHAD: Interaction Param",
    "CALPHAD: Phase Diagram", "CALPHAD: Scheil-Gulliver", "CALPHAD: Muggianu",
    "Phase-Field: Allen-Cahn", "Phase-Field: Cahn-Hilliard", "Phase-Field: KKS",
    "AI: PINN", "AI: Tensor NN", "AI: GPR", "AI: Active Learning", "AI: UQ"
]
...
EDGE_COLOR_REGISTRY: Dict[RelationshipType, str] = {
    ...
    RelationshipType.PROVIDES:          "#9370DB",
...
class DomainOntology:
    """Comprehensive ontology for Thermodynamic Data Tensor (Gibbs Tensor)."""
    ...
    def _build_ontology(self) -> None:
        ...
        self._add_concept("tensor_rank", ConceptType.PARAMETER,
            ...
            definition="Rank of a thermodynamic property tensor, controlling representation efficiency")
        self._add_concept("tucker_decomposition", ConceptType.METHOD,
            synonyms={"higher-order svd", "hosvd", "tucker", "tensor decomposition"},
            definition="Decomposition of a high-dimensional tensor into a core tensor and factor matrices")
        self._add_concept("tensor_completion", ConceptType.METHOD,
            ...
            definition="Recover missing thermodynamic data points via low-rank tensor approximation")
        self._add_concept("cp_decomposition", ConceptType.METHOD,
            synonyms={"canonical polyadic", "parallel factors", "parafac", "cp"},
            definition="Canonical Polyadic Decomposition expressing a tensor as sum of rank-one tensors")
        self._add_concept("core_tensor", ConceptType.PARAMETER,
            synonyms={"tensor core", "multibody interaction tensor", "core"},
            definition="Core tensor capturing multi-body interactions in the thermodynamic representation")
        self._add_concept("factor_matrix", ConceptType.PARAMETER,
            synonyms={"factor matrices", "loading matrix", "component matrix"},
            definition="Matrix of element-specific contributions to thermodynamic properties")
        ...
        self._add_concept("gibbs_free_energy", ConceptType.PROPERTY,
            synonyms={"gibbs energy", "free energy", "thermodynamic potential", "g"},
            definition="Gibbs free energy as a function of composition, temperature, and pressure")
        self._add_concept("excess_gibbs_energy", ConceptType.PROPERTY,
            synonyms={"g_xs", "excess gibbs", "non-ideal mixing", "excess free energy"},
            definition="Non-ideal mixing contribution to the Gibbs energy")
        self._add_concept("enthalpy_of_mixing", ConceptType.PROPERTY,
            synonyms={"delta h mix", "mixing enthalpy", "enthalpy of mixing"},
            definition="Enthalpy change upon mixing, composition-dependent")
        self._add_concept("entropy_of_mixing", ConceptType.PROPERTY,
            synonyms={"delta s mix", "mixing entropy", "configurational entropy"},
            definition="Entropy change upon mixing, composition-dependent")
        self._add_concept("chemical_potential", ConceptType.PROPERTY,
            synonyms={"mu_i", "partial molar gibbs energy", "diffusion potential"},
            definition="Partial molar Gibbs energy of component i")
        self._add_concept("activity_coefficient", ConceptType.PROPERTY,
            synonyms={"gamma_i", "thermodynamic activity", "raoultian activity"},
            definition="Deviation from ideal solution behaviour for component i")
        self._add_concept("heat_capacity", ConceptType.PROPERTY,
            synonyms={"cp", "specific heat", "thermal capacity"},
            definition="Heat capacity at constant pressure, temperature-dependent")
        self._add_concept("thermal_expansion", ConceptType.PROPERTY,
            synonyms={"cte", "coefficient of thermal expansion", "alpha"},
            definition="Relative volume change with temperature")

        # === CALPHAD & PHASE EQUILIBRIUM ===
        self._add_concept("calphad", ConceptType.METHOD,
            synonyms={"calculation of phase diagrams", "thermodynamic database", "calphad method"},
            definition="CALPHAD approach for modelling phase equilibria and thermodynamic properties")
        ...
        self._add_concept("sublattice_model", ConceptType.MODEL,
            synonyms={"compound energy formalism", "cem", "cel model", "site occupancy"},
        ...
        self._add_concept("interaction_parameter", ConceptType.PARAMETER,
            synonyms={"l_ij", "binary interaction", "redlich-kister coefficient"},
        ...
        self._add_concept("scheil_gulliver", ConceptType.METHOD,
            synonyms={"scheil", "scheil solidification", "non-equilibrium freezing"},
        ...
        self._add_concept("muggianu_extrapolation", ConceptType.METHOD,
            synonyms={"kohler model", "toop model", "geometric extrapolation"},
        ...
        self._add_concept("phase_diagram", ConceptType.METHOD,
            synonyms={"phase equilibria", "t-c diagram", "composition-temperature phase diagram"},
            definition="Map of stable phases as a function of composition and temperature")

        # === PHASE-FIELD & MICROSTRUCTURE ===
        self._add_concept("phase_field_model", ConceptType.MODEL,
            synonyms={"phase-field", "pf model", "diffuse interface"},
            definition="Computational model using order parameters to describe phase transitions")
        self._add_concept("allen_cahn_equation", ConceptType.MODEL,
            synonyms={"allen-cahn", "non-conserved dynamics", "order parameter evolution"},
            definition="Governing equation for non-conserved order parameters")
        self._add_concept("cahn_hilliard_equation", ConceptType.MODEL,
            synonyms={"cahn-hilliard", "conserved dynamics", "composition evolution"},
            definition="Governing equation for conserved composition fields with diffusion")
        self._add_concept("kks_model", ConceptType.MODEL,
            synonyms={"kim-kim-suzuki", "partitioning phase-field", "quantitative pf"},
        ...
        self._add_concept("grand_potential_formulation", ConceptType.MODEL,
            synonyms={"grand potential", "grand canonical", "omega potential"},
        ...
        self._add_concept("order_parameter", ConceptType.PARAMETER,
            synonyms={"eta", "phase field variable", "structural order parameter"},
        ...
        self._add_concept("physics_informed_neural_network", ConceptType.MODEL,
            synonyms={"pinn", "physics-informed nn", "physics-constrained ml"},
            definition="Neural network with physical law constraints embedded as loss terms")
        self._add_concept("tensor_neural_network", ConceptType.MODEL,
            synonyms={"tensor nn", "tensorized neural network", "tensor regression"},
            definition="Neural network architecture that operates on tensor-structured data")
        self._add_concept("gaussian_process_regression", ConceptType.MODEL,
            synonyms={"gpr", "kriging", "bayesian surrogate"},
        ...
        self._add_concept("active_learning", ConceptType.METHOD,
            ...
            definition="Iterative selection of most informative composition-temperature points for DFT/experiment")
        self._add_concept("bayesian_optimization", ConceptType.METHOD,
            synonyms={"bo", "sequential experimental design", "surrogate optimization"},
            definition="Global optimisation of expensive black-box functions with uncertainty guidance")
        self._add_concept("uncertainty_quantification", ConceptType.METHOD,
            synonyms={"uq", "sensitivity analysis", "propagation of uncertainty"},
        ...
        self._add_concept("cluster_expansion", ConceptType.METHOD,
            synonyms={"ce", "lattice hamiltonian", "ising expansion"},
        ...
        self._add_concept("special_quasirandom_structures", ConceptType.METHOD,
            synonyms={"sqs", "quasirandom", "disordered supercell"},
            definition="Periodic supercells mimicking random solid solution statistics")

        # === COMPOSITION & TEMPERATURE SPACE ===
        self._add_concept("composition_space", ConceptType.PARAMETER,
            synonyms={"compositional space", "alloy space", "simplex", "composition vector"},
            definition="Multi-dimensional space of possible compositions (simplex)")
        self._add_concept("temperature", ConceptType.PARAMETER,
            synonyms={"t", "thermal condition", "temperature variable"},
            definition="Temperature as an independent thermodynamic variable")
        self._add_concept("pressure", ConceptType.PARAMETER,
            synonyms={"p", "hydrostatic pressure", "external pressure"},
            definition="Pressure as an independent thermodynamic variable")
        self._add_concept("compositional_descriptor", ConceptType.PARAMETER,
            synonyms={"elemental fraction", "mole fraction", "atomic percent"},
            definition="Set of independent composition variables (usually n-1)")

        # === HIGH-DIMENSIONAL DATA & TENSOR PROPERTIES ===
        self._add_concept("high_dimensional_data", ConceptType.PARAMETER,
            synonyms={"curse of dimensionality", "high-dim", "many variables"},
            definition="Data with many independent variables (composition, T, P, etc.)")
        self._add_concept("thermodynamic_data_tensor", ConceptType.PROPERTY,
            synonyms={"property tensor", "thermodynamic tensor", "multi-component tensor"},
            definition="Tensor containing thermodynamic properties as function of composition, T, P")
        self._add_concept("gibbs_tensor", ConceptType.PROPERTY,
            synonyms={"gibbs tensor", "gibbs potential tensor", "free energy tensor"},
            definition="Tensor representation of Gibbs free energy over composition-temperature grid")
        ...
        self._add_concept("tensor_visualization", ConceptType.METHOD,
            synonyms={"tensor plot", "slice view", "multi-dim heatmap"},
            definition="Visualisation techniques for multi-dimensional thermodynamic data")

        # === MATERIALS & ALLOYS ===
        self._add_concept("multi_component_alloy", ConceptType.MATERIAL,
            synonyms={"mca", "complex concentrated alloy", "hea", "mea"},
            definition="Alloy with multiple principal elements")
        self._add_concept("binary_alloy", ConceptType.MATERIAL,
            synonyms={"binary system", "two-component alloy"},
            definition="Alloy of two elements")
        self._add_concept("ternary_alloy", ConceptType.MATERIAL,
            synonyms={"ternary system", "three-component alloy"},
            definition="Alloy of three elements")
        self._add_concept("quaternary_alloy", ConceptType.MATERIAL,
            synonyms={"quaternary system", "four-component alloy"},
            definition="Alloy of four elements")

        # === PHENOMENA ===
        self._add_concept("phase_stability", ConceptType.PHENOMENON,
            synonyms={"phase stability", "phase transition", "phase equilibrium"},
            definition="Stability of a phase as a function of composition and temperature")
        self._add_concept("spinodal_decomposition", ConceptType.PHENOMENON,
            synonyms={"spinodal", "phase separation", "unmixing"},
            definition="Diffusion-limited phase separation in the spinodal region")
        self._add_concept("nucleation", ConceptType.PHENOMENON,
            synonyms={"nucleation", "critical nucleus", "homogeneous nucleation"},
            definition="Formation of a new phase from a metastable parent phase")

        # === EVALUATION METRICS ===
        self._add_concept("mean_absolute_error", ConceptType.PARAMETER,
            synonyms={"mae", "l1 loss", "mean absolute deviation"},
            definition="Average absolute difference between predicted and true tensor entries")
        ...
        self._add_concept("relative_error", ConceptType.PARAMETER,
            synonyms={"relative error", "percentage error", "nrmse"},
            definition="Error normalised by the range of the data")
    ...
    def _add_concept(self, canonical_name: str, concept_type: ConceptType,
        ...
        node = ConceptNode(
            ...
            synonyms=synonyms or set(), hypernyms=hypernyms or set(),
    ...
    def _build_causal_chains(self) -> None:
        causal_chains = [
            # Tensor → Thermodynamic data
            ("tensor_completion", RelationshipType.INFLUENCES, "gibbs_free_energy", 0.90),
            ("cp_decomposition", RelationshipType.INFLUENCES, "thermodynamic_data_tensor", 0.85),
            ("tucker_decomposition", RelationshipType.INFLUENCES, "gibbs_tensor", 0.88),
            ("low_rank_approximation", RelationshipType.INFLUENCES, "excess_gibbs_energy", 0.80),
            ("alternating_least_squares", RelationshipType.DEPENDS_ON, "tensor_completion", 0.92),
            ("kronecker_product", RelationshipType.INFLUENCES, "factor_matrix", 0.75),
            ("core_tensor", RelationshipType.DEPENDS_ON, "tensor_rank", 0.85),
            ("factor_matrix", RelationshipType.PART_OF, "tucker_decomposition", 0.85),
            ("tensor_contraction", RelationshipType.COMPUTES, "thermodynamic_data_tensor", 0.80),
            # CALPHAD → Gibbs → Phase stability
            ("calphad", RelationshipType.COMPUTES, "gibbs_free_energy", 0.95),
            ("calphad", RelationshipType.COMPUTES, "phase_diagram", 0.95),
            ("redlich_kister_polynomials", RelationshipType.INFLUENCES, "excess_gibbs_energy", 0.90),
            ("sublattice_model", RelationshipType.INFLUENCES, "excess_gibbs_energy", 0.85),
            ("interaction_parameter", RelationshipType.INFLUENCES, "excess_gibbs_energy", 0.80),
            ("gibbs_free_energy", RelationshipType.CAUSES, "phase_stability", 0.95),
            ("excess_gibbs_energy", RelationshipType.CAUSES, "phase_stability", 0.90),
            ("enthalpy_of_mixing", RelationshipType.INFLUENCES, "phase_stability", 0.85),
            ("entropy_of_mixing", RelationshipType.INFLUENCES, "phase_stability", 0.85),
            ("gibbs_duhem_equation", RelationshipType.CONSTRAINS, "excess_gibbs_energy", 0.85),
            ("chemical_potential", RelationshipType.DEPENDS_ON, "gibbs_free_energy", 0.90),
            ("activity_coefficient", RelationshipType.INFLUENCES, "chemical_potential", 0.85),
            ("scheil_gulliver", RelationshipType.MODELS, "phase_diagram", 0.80),
            ("muggianu_extrapolation", RelationshipType.APPROXIMATES, "excess_gibbs_energy", 0.80),
            # Gibbs tensor → Composition & temperature dependence
            ("gibbs_tensor", RelationshipType.DEPENDS_ON, "composition_space", 0.90),
            ("gibbs_tensor", RelationshipType.DEPENDS_ON, "temperature", 0.90),
            ("gibbs_tensor", RelationshipType.DEPENDS_ON, "pressure", 0.70),
            ("composition_space", RelationshipType.INFLUENCES, "phase_stability", 0.80),
            ("temperature", RelationshipType.INFLUENCES, "phase_stability", 0.80),
            ("compositional_descriptor", RelationshipType.INFLUENCES, "gibbs_tensor", 0.85),
            ("compositional_descriptor", RelationshipType.INFLUENCES, "phase_diagram", 0.80),
            ("heat_capacity", RelationshipType.DEPENDS_ON, "temperature", 0.85),
            ("thermal_expansion", RelationshipType.DEPENDS_ON, "temperature", 0.85),
            # DFT → CALPHAD parameters
            ("density_functional_theory", RelationshipType.COMPUTES, "enthalpy_of_mixing", 0.85),
            ("density_functional_theory", RelationshipType.COMPUTES, "interaction_parameter", 0.80),
            ("cluster_expansion", RelationshipType.MAPS, "density_functional_theory", 0.85),
            ("special_quasirandom_structures", RelationshipType.MODELS, "density_functional_theory", 0.80),
            ("molecular_dynamics", RelationshipType.SIMULATES, "heat_capacity", 0.75),
            # Phase-field ↔ Thermodynamics
            ("phase_field_model", RelationshipType.DEPENDS_ON, "gibbs_free_energy", 0.90),
            ("phase_field_model", RelationshipType.DEPENDS_ON, "chemical_potential", 0.85),
            ("allen_cahn_equation", RelationshipType.INFLUENCES, "phase_field_model", 0.95),
            ("cahn_hilliard_equation", RelationshipType.INFLUENCES, "phase_field_model", 0.95),
            ("cahn_hilliard_equation", RelationshipType.CAUSES, "spinodal_decomposition", 0.90),
            ("kks_model", RelationshipType.MODELS, "phase_field_model", 0.85),
            ("grand_potential_formulation", RelationshipType.MODELS, "phase_field_model", 0.85),
            ("order_parameter", RelationshipType.DEPENDS_ON, "allen_cahn_equation", 0.90),
            # Machine learning → Surrogate modeling
            ("physics_informed_neural_network", RelationshipType.LEARNS, "gibbs_free_energy", 0.85),
            ("physics_informed_neural_network", RelationshipType.ENFORCES, "gibbs_duhem_equation", 0.80),
            ("tensor_neural_network", RelationshipType.LEARNS, "thermodynamic_data_tensor", 0.80),
            ("gaussian_process_regression", RelationshipType.APPROXIMATES, "gibbs_free_energy", 0.75),
            ("active_learning", RelationshipType.QUERIES, "gaussian_process_regression", 0.80),
            ("bayesian_optimization", RelationshipType.OPTIMIZES, "calphad", 0.75),
            # Uncertainty
            ("uncertainty_quantification", RelationshipType.CHARACTERIZES, "gibbs_free_energy", 0.85),
            ("uncertainty_quantification", RelationshipType.QUANTIFIES, "phase_stability", 0.80),
            ("gaussian_process_regression", RelationshipType.PROVIDES, "uncertainty_quantification", 0.85),
            # Evaluation
            ("mean_absolute_error", RelationshipType.EVALUATES, "tensor_completion", 0.80),
            ("root_mean_square_error", RelationshipType.EVALUATES, "tensor_completion", 0.80),
            ("r2_score", RelationshipType.EVALUATES, "physics_informed_neural_network", 0.85),
            ("relative_error", RelationshipType.COMPARES, "tucker_decomposition", 0.75),
            # Multi-scale bridging
            ("density_functional_theory", RelationshipType.TRAINS, "physics_informed_neural_network", 0.80),
            ("molecular_dynamics", RelationshipType.SIMULATES, "phase_field_model", 0.75),
            ("calphad", RelationshipType.UPSCALES, "phase_field_model", 0.80),
            ("calphad", RelationshipType.INTEGRATES, "density_functional_theory", 0.85),
            # High-dimensional data
            ("high_dimensional_data", RelationshipType.INFLUENCES, "tensor_completion", 0.85),
            ("thermodynamic_data_tensor", RelationshipType.DEPENDS_ON, "high_dimensional_data", 0.80),
            ("gibbs_tensor", RelationshipType.PART_OF, "thermodynamic_data_tensor", 0.95),
            ("tensor_visualization", RelationshipType.VISUALIZES, "gibbs_tensor", 0.85),
            # Material hierarchy
            ("binary_alloy", RelationshipType.HYPONYM, "multi_component_alloy", 1.0),
            ("ternary_alloy", RelationshipType.HYPONYM, "multi_component_alloy", 1.0),
            ("quaternary_alloy", RelationshipType.HYPONYM, "multi_component_alloy", 1.0),
            # Nucleation
            ("gibbs_free_energy", RelationshipType.DRIVES, "nucleation", 0.85),
            ("chemical_potential", RelationshipType.INFLUENCES, "nucleation", 0.75),
        ]
    ...
    def get_related_concepts(self, canonical_name: str, rel_type: RelationshipType = None
...
# ----------------------------------------------------------------------------
# HIERARCHY LABEL BUILDER
# ----------------------------------------------------------------------------
_HIERARCHY_PARENTS: Dict[str, Tuple[str, int]] = {
    # Root
    "thermodynamic_data_tensor": (None, 0),
    "gibbs_tensor": (None, 0),
    # Tier 1: Major categories
    "tensor_completion": ("Tensor Methods", 1),
    "tucker_decomposition": ("Tensor Methods", 1),
    "cp_decomposition": ("Tensor Methods", 1),
    "low_rank_approximation": ("Tensor Methods", 1),
    "alternating_least_squares": ("Tensor Methods", 1),
    "kronecker_product": ("Tensor Methods", 1),
    "tensor_contraction": ("Tensor Methods", 1),
    "tensor_rank": ("Tensor Methods", 1),
    "core_tensor": ("Tensor Methods", 1),
    "factor_matrix": ("Tensor Methods", 1),
    "gibbs_free_energy": ("Thermodynamic Properties", 1),
    "excess_gibbs_energy": ("Thermodynamic Properties", 1),
    "enthalpy_of_mixing": ("Thermodynamic Properties", 1),
    "entropy_of_mixing": ("Thermodynamic Properties", 1),
    "chemical_potential": ("Thermodynamic Properties", 1),
    "activity_coefficient": ("Thermodynamic Properties", 1),
    "heat_capacity": ("Thermodynamic Properties", 1),
    "thermal_expansion": ("Thermodynamic Properties", 1),
    "calphad": ("CALPHAD Methods", 1),
    "redlich_kister_polynomials": ("CALPHAD Methods", 1),
    "sublattice_model": ("CALPHAD Methods", 1),
    "interaction_parameter": ("CALPHAD Methods", 1),
    "phase_diagram": ("CALPHAD Methods", 1),
    ...
    "physics_informed_neural_network": ("AI & Data-Driven", 1),
    "tensor_neural_network": ("AI & Data-Driven", 1),
    "gaussian_process_regression": ("AI & Data-Driven", 1),
    "active_learning": ("AI & Data-Driven", 1),
    "bayesian_optimization": ("AI & Data-Driven", 1),
    "uncertainty_quantification": ("AI & Data-Driven", 1),
    ...
    "composition_space": ("Thermodynamic Variables", 1),
    "temperature": ("Thermodynamic Variables", 1),
    "pressure": ("Thermodynamic Variables", 1),
    "compositional_descriptor": ("Thermodynamic Variables", 1),
    "high_dimensional_data": ("Data & Representation", 1),
    "tensor_visualization": ("Data & Representation", 1),
    "phase_stability": ("Phenomena", 1),
    "spinodal_decomposition": ("Phenomena", 1),
    "nucleation": ("Phenomena", 1),
    "mean_absolute_error": ("Evaluation Metrics", 1),
    "root_mean_square_error": ("Evaluation Metrics", 1),
    "r2_score": ("Evaluation Metrics", 1),
    "relative_error": ("Evaluation Metrics", 1),
    "multi_component_alloy": ("Materials", 1),
    "binary_alloy": ("Materials", 1),
    "ternary_alloy": ("Materials", 1),
    "quaternary_alloy": ("Materials", 1),
}
...
def get_hierarchy_path(concept_key: str) -> List[str]:
    ...
    if entry is None or entry[0] is None:
        return ["Thermodynamic Data Tensor", leaf]
    parent_label = entry[0]
    return ["Thermodynamic Data Tensor", parent_label, leaf]
...
def build_sunburst_data(graph: nx.Graph, node_weights: Optional[Dict[str, float]] = None,
    ...
    root_id = "Thermodynamic Data Tensor"
    ids.append(root_id)
    labels.append("Thermodynamic Data Tensor")
    values.append(0)
    parents.append("")
...
# ----------------------------------------------------------------------------
# TENSOR THERMODYNAMICS PROBLEM DEFINITIONS
# ----------------------------------------------------------------------------
class TensorThermodynamicsProblem(Enum):
    TENSOR_COMPLETION = "tensor_completion"
    TENSOR_DECOMPOSITION = "tensor_decomposition"
    GIBBS_ENERGY_PREDICTION = "gibbs_energy_prediction"
    PHASE_DIAGRAM_PREDICTION = "phase_diagram_prediction"
    UNCERTAINTY_ANALYSIS = "uncertainty_analysis"
    CALPHAD_OPTIMIZATION = "calphad_optimization"
...
@dataclass
class TensorProblemDefinition:
    problem_id: TensorThermodynamicsProblem
    title: str
...
TENSOR_PROBLEM_DEFINITIONS: Dict[TensorThermodynamicsProblem, TensorProblemDefinition] = {
    TensorThermodynamicsProblem.TENSOR_COMPLETION: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.TENSOR_COMPLETION,
        title="Thermodynamic Tensor Completion",
        scientific_description="Recover missing values in a thermodynamic property tensor (e.g., Gibbs free energy) using low-rank tensor decomposition techniques.",
        root_cause="Sparse experimental or DFT data due to high-dimensional composition-temperature space.",
        key_concepts=["tensor_completion", "cp_decomposition", "tucker_decomposition", "low_rank_approximation", "alternating_least_squares"],
        key_relationships=[("tensor_completion", "DEPENDS_ON", "low_rank_approximation"),
                           ("cp_decomposition", "INFLUENCES", "tensor_completion")],
        solution_directions=["Apply CP or Tucker decomposition", "Use ALS for optimisation", "Incorporate physical constraints"],
        relevant_descriptors=["tensor_rank", "factor_matrix", "core_tensor"],
        relevant_phenomena=["phase_stability", "spinodal_decomposition"],
        relevant_properties=["gibbs_free_energy", "excess_gibbs_energy"],
        example_queries=["How to complete missing Gibbs energy data for a ternary alloy?",
                         "Which tensor decomposition is best for thermodynamic data?"],
        visualization_focus=["tensor_slice_plots", "error_heatmap"]
    ),
    TensorThermodynamicsProblem.TENSOR_DECOMPOSITION: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.TENSOR_DECOMPOSITION,
        title="Tensor Decomposition for Thermodynamic Data",
        scientific_description="Decompose high-dimensional Gibbs tensor into core tensor and factor matrices for compact representation.",
        root_cause="High-dimensional data over composition-temperature space is hard to store and analyse.",
        key_concepts=["tucker_decomposition", "cp_decomposition", "core_tensor", "factor_matrix", "tensor_rank"],
        key_relationships=[("tucker_decomposition", "INFLUENCES", "gibbs_tensor"),
                           ("cp_decomposition", "INFLUENCES", "thermodynamic_data_tensor")],
        solution_directions=["Tucker with rank selection", "CP with ALS", "Compare reconstruction error"],
        relevant_descriptors=["tensor_rank", "core_tensor", "factor_matrix"],
        relevant_phenomena=[],
        relevant_properties=["gibbs_tensor", "thermodynamic_data_tensor"],
        example_queries=["What is the optimal tensor rank for representing Gibbs energy?",
                         "How does Tucker decomposition compress thermodynamic data?"],
        visualization_focus=["core_tensor_heatmap", "factor_matrix_plot"]
    ),
    TensorThermodynamicsProblem.GIBBS_ENERGY_PREDICTION: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.GIBBS_ENERGY_PREDICTION,
        title="Gibbs Free Energy Prediction from Composition & Temperature",
        scientific_description="Build surrogate models to predict Gibbs free energy as a function of composition and temperature using tensor-based neural networks or Gaussian processes.",
        root_cause="Computationally expensive DFT/CALPHAD calculations.",
        key_concepts=["gibbs_free_energy", "physics_informed_neural_network", "gaussian_process_regression", "composition_space", "temperature"],
        key_relationships=[("gibbs_free_energy", "DEPENDS_ON", "composition_space"),
                           ("gibbs_free_energy", "DEPENDS_ON", "temperature"),
                           ("physics_informed_neural_network", "LEARNS", "gibbs_free_energy")],
        solution_directions=["PINN with thermodynamic constraints", "GPR with kernel engineering", "Tensor-based regression"],
        relevant_descriptors=["compositional_descriptor", "temperature", "pressure"],
        relevant_phenomena=["phase_stability"],
        relevant_properties=["gibbs_free_energy", "enthalpy_of_mixing", "entropy_of_mixing"],
        example_queries=["How to predict Gibbs energy for arbitrary Co-Cr-Fe-Ni compositions?",
                         "Which ML model works best for temperature-dependent thermodynamics?"],
        visualization_focus=["gibbs_surface_plot", "error_distribution"]
    ),
    TensorThermodynamicsProblem.PHASE_DIAGRAM_PREDICTION: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.PHASE_DIAGRAM_PREDICTION,
        title="Phase Diagram Prediction from Tensor Models",
        scientific_description="Use tensor-represented Gibbs energy to predict phase boundaries and stability regions as function of composition and temperature.",
        root_cause="CALPHAD databases are expensive to construct; tensor completion can accelerate phase diagram construction.",
        key_concepts=["phase_diagram", "gibbs_tensor", "calphad", "phase_stability", "composition_space"],
        key_relationships=[("gibbs_tensor", "INFLUENCES", "phase_diagram"),
                           ("calphad", "COMPUTES", "phase_diagram")],
        solution_directions=["Tensor completion to fill CALPHAD data", "Use convex hull for phase boundaries", "Couple with uncertainty quantification"],
        relevant_descriptors=["gibbs_free_energy", "excess_gibbs_energy", "interaction_parameter"],
        relevant_phenomena=["phase_stability", "spinodal_decomposition"],
        relevant_properties=["phase_diagram"],
        example_queries=["Can tensor completion predict the phase diagram of a quaternary alloy?",
                         "How to reconstruct phase boundaries from sparse data?"],
        visualization_focus=["phase_diagram_2D", "tensor_slice_plot"]
    ),
    TensorThermodynamicsProblem.UNCERTAINTY_ANALYSIS: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.UNCERTAINTY_ANALYSIS,
        title="Uncertainty Quantification in Tensor Models",
        scientific_description="Assess epistemic and aleatoric uncertainties in tensor-based thermodynamic models due to sparse data and model approximations.",
        root_cause="Limited training data and tensor rank selection.",
        key_concepts=["uncertainty_quantification", "gaussian_process_regression", "tensor_rank", "low_rank_approximation"],
        key_relationships=[("uncertainty_quantification", "QUANTIFIES", "gibbs_free_energy"),
                           ("gaussian_process_regression", "PROVIDES", "uncertainty_quantification")],
        solution_directions=["GP with uncertainty propagation", "Bootstrap resampling", "Bayesian tensor decomposition"],
        relevant_descriptors=["tensor_rank", "factor_matrix"],
        relevant_phenomena=[],
        relevant_properties=["gibbs_free_energy", "excess_gibbs_energy"],
        example_queries=["What is the uncertainty in Gibbs energy prediction from incomplete data?",
                         "How to quantify rank selection uncertainty?"],
        visualization_focus=["confidence_intervals", "error_bars"]
    ),
    TensorThermodynamicsProblem.CALPHAD_OPTIMIZATION: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.CALPHAD_OPTIMIZATION,
        title="CALPHAD Database Optimisation via Tensor Methods",
        scientific_description="Optimise CALPHAD interaction parameters and Redlich-Kister coefficients using tensor completion and Bayesian optimisation.",
        root_cause="Manual CALPHAD assessment is laborious; tensor methods can accelerate parameter optimisation.",
        key_concepts=["calphad", "redlich_kister_polynomials", "interaction_parameter", "bayesian_optimization", "excess_gibbs_energy"],
        key_relationships=[("redlich_kister_polynomials", "INFLUENCES", "excess_gibbs_energy"),
                           ("bayesian_optimization", "OPTIMIZES", "calphad")],
        solution_directions=["Bayesian optimisation of L_ij parameters", "Tensor completion of binary data", "Active learning for missing binaries"],
        relevant_descriptors=["interaction_parameter", "compositional_descriptor"],
        relevant_phenomena=["phase_stability"],
        relevant_properties=["excess_gibbs_energy", "gibbs_free_energy", "phase_diagram"],
        example_queries=["How to optimise Redlich-Kister parameters with Bayesian optimisation?",
                         "Can tensor methods accelerate CALPHAD database assessment?"],
        visualization_focus=["parameter_convergence", "excess_gibbs_landscape"]
    ),
    TensorThermodynamicsProblem.MULTI_SCALE_COUPLING: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.MULTI_SCALE_COUPLING,
        title="Multi-Scale Coupling: DFT → CALPHAD → Phase-Field",
        scientific_description="Bridge DFT, CALPHAD, and phase-field modelling via tensor representations of thermodynamic properties.",
        root_cause="Each scale produces data in different formats; tensors provide a unifying representation.",
        key_concepts=["density_functional_theory", "calphad", "phase_field_model", "gibbs_tensor", "cluster_expansion"],
        key_relationships=[("density_functional_theory", "COMPUTES", "enthalpy_of_mixing"),
                           ("calphad", "UPSCALES", "phase_field_model"),
                           ("cluster_expansion", "MAPS", "density_functional_theory")],
        solution_directions=["Cluster expansion for DFT-to-CALPHAD", "Gibbs tensor as common interface", "PINN surrogate for phase-field"],
        relevant_descriptors=["special_quasirandom_structures", "interaction_parameter"],
        relevant_phenomena=["phase_stability", "spinodal_decomposition", "nucleation"],
        relevant_properties=["gibbs_free_energy", "chemical_potential", "heat_capacity"],
        example_queries=["How to couple DFT data with CALPHAD via tensors?",
                         "What is the role of cluster expansion in multi-scale thermodynamics?"],
        visualization_focus=["multi_scale_pipeline", "gibbs_tensor_slices"]
    ),
    TensorThermodynamicsProblem.GENERAL: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.GENERAL,
        title="General Thermodynamic Tensor Inquiry",
        scientific_description="General question about thermodynamic data tensors or Gibbs tensor representation.",
        root_cause="N/A",
        key_concepts=["thermodynamic_data_tensor"],
        key_relationships=[],
        solution_directions=[],
        relevant_descriptors=[],
        relevant_phenomena=[],
        relevant_properties=[],
        example_queries=["What is a thermodynamic data tensor?"],
        visualization_focus=["general_overview"]
    ),
    TensorThermodynamicsProblem.MULTI_PROBLEM: TensorProblemDefinition(
        problem_id=TensorThermodynamicsProblem.MULTI_PROBLEM,
        title="Multi-Problem Thermodynamic Tensor Inquiry",
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        TensorThermodynamicsProblem.TENSOR_COMPLETION: {"tensor completion", "missing data", "low-rank", "cp", "tucker"},
        TensorThermodynamicsProblem.TENSOR_DECOMPOSITION: {"decomposition", "tucker", "cp", "parafac", "rank"},
        TensorThermodynamicsProblem.GIBBS_ENERGY_PREDICTION: {"gibbs", "free energy", "surrogate", "pinn", "gpr", "prediction"},
        TensorThermodynamicsProblem.PHASE_DIAGRAM_PREDICTION: {"phase diagram", "phase boundary", "calphad", "phase stability"},
        TensorThermodynamicsProblem.UNCERTAINTY_ANALYSIS: {"uncertainty", "confidence", "error", "gp", "uq"},
        TensorThermodynamicsProblem.CALPHAD_OPTIMIZATION: {"calphad", "optimize", "database", "parameters", "redlich"},
        TensorThermodynamicsProblem.MULTI_SCALE_COUPLING: {"multi-scale", "dft", "md", "phase-field", "coupling"},
        TensorThermodynamicsProblem.GENERAL: {"thermodynamics", "tensor", "alloy", "composition", "temperature"},
    }
    def is_available(self) -> bool:
        return True
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        primary = max(problem_scores, key=problem_scores.get) if sum(problem_scores.values()) > 0 else TensorThermodynamicsProblem.GENERAL
        ...
        inferred = []
        if primary != TensorThermodynamicsProblem.GENERAL:
            pdef = TENSOR_PROBLEM_DEFINITIONS[primary]
        ...
        priorities = {}
        pdef = TENSOR_PROBLEM_DEFINITIONS.get(primary, TENSOR_PROBLEM_DEFINITIONS[TensorThermodynamicsProblem.GENERAL])
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in thermodynamic data tensors (Gibbs tensor) and CALPHAD methods. Analyze the user's query and return ONLY valid JSON with:
        1. "primary_problem": One of: tensor_completion, tensor_decomposition, gibbs_energy_prediction, phase_diagram_prediction, uncertainty_analysis, calphad_optimization, multi_scale_coupling, general, multi_problem
        ...
        try:
            ...
            problem_map = {p.value: p for p in TensorThermodynamicsProblem}
            primary = problem_map.get(parsed.get("primary_problem", "general"), TensorThermodynamicsProblem.GENERAL)
            ...
            return QueryAnalysisResult(
                ...
                visualization_focus=TENSOR_PROBLEM_DEFINITIONS[primary].visualization_focus,
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        if not self.is_available():
            return FallbackAnalyzer().analyze_query(query, ontology)
        prompt = f"[INST] You are an expert in thermodynamic data tensors. Analyze: '{query}'. Return ONLY valid JSON with: primary_problem, explicitly_mentioned (snake_case list), inferred_concepts (list), query_type, highlight_paths (list of [src, tgt]), reasoning_chain (list). [/INST]"
        try:
...
class DynamicOntologyExpander:
    ...
    def _create_bridge_concept(self, missing_concept: str, source_query: str, problem: TensorThermodynamicsProblem) -> Optional[Dict]:
        bridge_name = f"query_bridge_{missing_concept.replace(' ', '_').lower()}"
        if bridge_name in self.ontology.concepts:
            return None
        pdef = TENSOR_PROBLEM_DEFINITIONS.get(problem, TENSOR_PROBLEM_DEFINITIONS[TensorThermodynamicsProblem.GENERAL])
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph,
        ...
        nl = chr(10)
        prompt = "You are an expert in thermodynamic data tensors (Gibbs tensor) and CALPHAD methods. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        if evidence_snippets:
            ...
            prompt += "No direct text snippets found. Rely on your general knowledge of thermodynamic tensors but note the lack of specific retrieved context." + nl
        prompt += "Instructions:" + nl
        prompt += "1. Provide a direct, scientifically accurate answer (2-3 paragraphs)." + nl
        prompt += "2. Explicitly mention how the key concepts interact (e.g., causal chains like 'tensor completion influences Gibbs energy accuracy')." + nl
    ...
    def _call_llm_for_answer(self, prompt: str, analyzer: LLMQueryAnalyzer, query: str, analysis: QueryAnalysisResult, top_nodes, evidence_snippets) -> str:
        ...
        if client:
            try:
                response = client.chat.completions.create(
                    model=analyzer.model, messages=[{"role": "user", "content": prompt}],
...
class QuerySessionManager:
    SESSION_KEY = "tensor_query_session"
    @classmethod
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(self, text: str, context: str) -> Optional[str]:
        ...
        tensor_indicators = ['tensor', 'decomposition', 'rank', 'completion', 'tucker', 'cp']
        thermo_indicators = ['gibbs', 'energy', 'enthalpy', 'entropy', 'free energy', 'calphad']
        phase_indicators = ['phase', 'diagram', 'stability', 'field']
        is_tensor = any(ind in context_lower for ind in tensor_indicators)
        is_thermo = any(ind in context_lower for ind in thermo_indicators)
        is_phase = any(ind in context_lower for ind in phase_indicators)
        if 'phase' in text:
            if is_thermo and not is_phase:
        ...
        if 'tensor' in text:
            if is_thermo:
                ...
                return "thermodynamic_data_tensor"
        return None
...
# ----------------------------------------------------------------------------
# TENSOR KEYWORDS & PATTERNS
# ----------------------------------------------------------------------------
TENSOR_KEYWORDS = [
    "tensor", "tensor decomposition", "tensor completion", "tensor rank", "cp decomposition",
    "tucker decomposition", "core tensor", "factor matrix", "alternating least squares",
    "low-rank", "kronecker product", "mode-n product", "tensor contraction",
    "thermodynamic data tensor", "gibbs tensor", "gibbs free energy", "excess gibbs",
    "enthalpy of mixing", "entropy of mixing", "chemical potential", "activity coefficient",
    "calphad", "redlich-kister", "sublattice model", "interaction parameter",
    "phase diagram", "phase stability", "scheil-gulliver", "muggianu", "gibbs-duhem",
    "composition space", "temperature", "pressure", "high-dimensional data",
    "phase-field", "allen-cahn", "cahn-hilliard", "kks model", "grand potential",
    "order parameter", "physics-informed neural network", "tensor neural network",
    "gaussian process", "active learning", "bayesian optimization", "uncertainty quantification",
    "density functional theory", "molecular dynamics", "cluster expansion", "sqs",
    "dft", "md", "first-principles", "ab initio"
]
...
TENSOR_DESCRIPTOR_MAPPING = {
    ...
    r'enthalpy|entropy|mixing|chemical\s+potential|activity\s+coefficient': 'thermodynamic_property',
    r'calphad|redlich-kister|sublattice|interaction\s+parameter|phase\s+diagram|scheil|muggianu|gibbs-duhem': 'calphad_method',
    r'composition\s+space|temperature|pressure|high-dimensional': 'thermodynamic_variable',
    r'phase-field|allen-cahn|cahn-hilliard|kks|grand\s+potential|order\s+parameter': 'phase_field_model',
...
TENSOR_QUANTITATIVE_PATTERNS = [
    r'\btensor\s+(?:decomposition|completion|rank|contraction|product)\b',
    r'\bgibbs\s+(?:free\s+energy|tensor)\b',
    r'\b(?:enthalpy|entropy)\s+of\s+mixing\b',
    r'\bchemical\s+potential\b',
    r'\bactivity\s+coefficient\b',
    r'\bcalphad\b',
    r'\bredlich-kister\b',
    r'\bsublattice\s+model\b',
    r'\binteraction\s+parameter\b',
    r'\bphase\s+(?:diagram|stability|equilibrium)\b',
    r'\bscheil-gulliver\b',
    r'\bmuggianu\b',
    r'\bgibbs-duhem\b',
    r'\bphase-field\b',
    r'\ballen-cahn\b',
    r'\bcahn-hilliard\b',
    r'\bkks\s+model\b',
    r'\bgrand\s+potential\b',
    r'\border\s+parameter\b',
    r'\bphysics-informed\s+neural\s+network\b',
    r'\btensor\s+neural\s+network\b',
    r'\bgaussian\s+process\b',
    r'\bactive\s+learning\b',
    r'\bbayesian\s+optimization\b',
    r'\buncertainty\s+quantification\b',
    r'\bdensity\s+functional\s+theory\b',
    r'\bmolecular\s+dynamics\b',
    r'\bcluster\s+expansion\b',
    r'\bsqs\b',
    r'\bab\s+initio\b',
    r'\bfirst-principles\b',
    r'\btensor\s+(?:core|factor\s+matrix|low-rank)\b',
    r'\bcanonical\s+polyadic\b',
    r'\btucker\s+decomposition\b',
    r'\balternating\s+least\s+squares\b',
    r'\bkronecker\s+product\b',
]

ALL_TENSOR_KEYWORDS = TENSOR_KEYWORDS
...
def categorize_tensor_concept(concept: str) -> str:
    concept_lower = concept.lower()
    for pattern, category in TENSOR_DESCRIPTOR_MAPPING.items():
        if re.search(pattern, concept_lower):
            return category
    return "general"


def get_tensor_category_color(concept: str, cmap_colors=None) -> str:
    if cmap_colors:
        return cmap_colors[hash(concept) % len(cmap_colors)]
    cat = categorize_tensor_concept(concept)
    color_map = {
        'tensor_method': '#1f77b4',
        'thermodynamic_property': '#ff7f0e',
        'calphad_method': '#2ca02c',
        'thermodynamic_variable': '#d62728',
        'phase_field_model': '#9467bd',
        'ai_method': '#8c564b',
        'multi_scale_method': '#e377c2',
        'thermophysical_property': '#7f7f7f',
        'phenomena': '#bcbd22',
        'evaluation_metric': '#17becf',
        'materials': '#aec7e8',
        'general': '#c5b0d5',
    }
...
def extract_tensor_concepts_from_text(text: str) -> List[str]:
    concepts = set()
    text_lower = text.lower()
    for pattern in TENSOR_QUANTITATIVE_PATTERNS:
...
def is_valid_tensor_concept(concept: str) -> bool:
    concept_lower = concept.lower()
    has_domain = any(kw in concept_lower for kw in TENSOR_KEYWORDS)
    has_pattern = any(re.search(p, concept, re.I) for p in TENSOR_QUANTITATIVE_PATTERNS)
...
def normalize_tensor_concept(concept: str) -> str:
    concept = concept.lower().strip()
    concept = re.sub(r'\btensor\s+completion\b', 'tensor_completion', concept)
    concept = re.sub(r'\btucker\s+decomposition\b', 'tucker_decomposition', concept)
    concept = re.sub(r'\bcanonical\s+polyadic\b', 'cp_decomposition', concept)
    concept = re.sub(r'\bgibbs\s+free\s+energy\b', 'gibbs_free_energy', concept)
    concept = re.sub(r'\bexcess\s+gibbs\s+energy\b', 'excess_gibbs_energy', concept)
    concept = re.sub(r'\benthalpy\s+of\s+mixing\b', 'enthalpy_of_mixing', concept)
    concept = re.sub(r'\bentropy\s+of\s+mixing\b', 'entropy_of_mixing', concept)
    concept = re.sub(r'\bchemical\s+potential\b', 'chemical_potential', concept)
    concept = re.sub(r'\bphase\s+diagram\b', 'phase_diagram', concept)
    concept = re.sub(r'\bphase\s+stability\b', 'phase_stability', concept)
    concept = re.sub(r'\bphysics-informed\s+neural\s+network\b', 'physics_informed_neural_network', concept)
    concept = re.sub(r'\btensor\s+neural\s+network\b', 'tensor_neural_network', concept)
    concept = re.sub(r'\bgaussian\s+process\b', 'gaussian_process_regression', concept)
    concept = re.sub(r'\bactive\s+learning\b', 'active_learning', concept)
    concept = re.sub(r'\bbayesian\s+optimization\b', 'bayesian_optimization', concept)
    concept = re.sub(r'\buncertainty\s+quantification\b', 'uncertainty_quantification', concept)
    concept = re.sub(r'\bdensity\s+functional\s+theory\b', 'density_functional_theory', concept)
    concept = re.sub(r'\bmolecular\s+dynamics\b', 'molecular_dynamics', concept)
    concept = re.sub(r'\bcluster\s+expansion\b', 'cluster_expansion', concept)
    concept = re.sub(r'\bsqs\b', 'special_quasirandom_structures', concept)
    concept = re.sub(r'\bab\s+initio\b', 'ab_initio', concept)
    concept = re.sub(r'\bfirst-principles\b', 'first_principles', concept)
    concept = re.sub(r'\bphase-field\b', 'phase_field_model', concept)
    concept = re.sub(r'\ballen-cahn\b', 'allen_cahn_equation', concept)
    concept = re.sub(r'\bcahn-hilliard\b', 'cahn_hilliard_equation', concept)
    concept = re.sub(r'\bkks\s+model\b', 'kks_model', concept)
    concept = re.sub(r'\bgrand\s+potential\b', 'grand_potential_formulation', concept)
    concept = re.sub(r'\border\s+parameter\b', 'order_parameter', concept)
    concept = re.sub(r'\bredlich-kister\b', 'redlich_kister_polynomials', concept)
    concept = re.sub(r'\bsublattice\s+model\b', 'sublattice_model', concept)
    concept = re.sub(r'\binteraction\s+parameter\b', 'interaction_parameter', concept)
    concept = re.sub(r'\bscheil-gulliver\b', 'scheil_gulliver', concept)
    concept = re.sub(r'\bmuggianu\b', 'muggianu_extrapolation', concept)
    concept = re.sub(r'\bgibbs-duhem\b', 'gibbs_duhem_equation', concept)
    concept = re.sub(r'\btensor\s+rank\b', 'tensor_rank', concept)
    concept = re.sub(r'\bcore\s+tensor\b', 'core_tensor', concept)
    concept = re.sub(r'\bfactor\s+matrix\b', 'factor_matrix', concept)
    concept = re.sub(r'\bkronecker\s+product\b', 'kronecker_product', concept)
    concept = re.sub(r'\btensor\s+contraction\b', 'tensor_contraction', concept)
    concept = re.sub(r'\balternating\s+least\s+squares\b', 'alternating_least_squares', concept)
    concept = re.sub(r'\blow-rank\b', 'low_rank_approximation', concept)
    concept = re.sub(r'\bheat\s+capacity\b', 'heat_capacity', concept)
    concept = re.sub(r'\bthermal\s+expansion\b', 'thermal_expansion', concept)
    concept = re.sub(r'\bactivity\s+coefficient\b', 'activity_coefficient', concept)
    concept = re.sub(r'\bcomposition\s+space\b', 'composition_space', concept)
    concept = re.sub(r'\bcompositional\s+descriptor\b', 'compositional_descriptor', concept)
    concept = re.sub(r'\bmulti-component\s+alloy\b', 'multi_component_alloy', concept)
    concept = re.sub(r'\bbinary\s+alloy\b', 'binary_alloy', concept)
    concept = re.sub(r'\bternary\s+alloy\b', 'ternary_alloy', concept)
    concept = re.sub(r'\bquaternary\s+alloy\b', 'quaternary_alloy', concept)
    return concept
...
def extract_concepts_from_abstracts(df: pd.DataFrame, text_columns: List[str]) -> Tuple[List[List[str]], List[Dict]]:
    ...
    for idx, row in df.iterrows():
        ...
        energy_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:j|kj|mj|ev)', combined_text, re.I)
        if energy_matches:
            metrics['energy'] = [float(m) for m in energy_matches]
        pressure_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mpa|gpa|pa|kpa)', combined_text, re.I)
        if pressure_matches:
            metrics['pressure'] = [float(m) for m in pressure_matches]
        all_metrics.append(metrics)
        concepts = extract_concepts_from_text(combined_text)
        normalized = [normalize_tensor_concept(c) for c in concepts]
...
def cluster_similar_concepts(valid_concepts: List[str], embed_model, similarity_threshold: float = 0.75) -> Tuple[List[str], Dict[str, str]]:
    ...
    try:
        ...
        clustering = AgglomerativeClustering(n_clusters=None, distance_threshold=1 - similarity_threshold, linkage='average', metric='cosine').fit(embeddings)
        ...
        for label, members in cluster_members.items():
            def score(m):
                domain_hits = sum(1 for kw in ALL_TENSOR_KEYWORDS if kw.lower() in m.lower())
...
def normalize_and_filter_concepts(all_concepts: List[List[str]], config: Dict) -> Tuple[List[str], Dict[str, int], Dict[int, str], Dict[str, List[int]]]:
    ...
    for doc_idx, concepts in enumerate(all_concepts):
        ...
        for c in concepts:
            if c not in seen_in_doc and is_valid_tensor_concept(c):
                concept_counts[c] += 1
    ...
    if config.get("USE_SEMANTIC_CLUSTERING", False) and len(valid_concepts) > 50:
        try:
            ...
            valid_concepts, concept_to_cluster = cluster_similar_concepts(valid_concepts, embed_model, similarity_threshold=config.get("CLUSTER_SIMILARITY", 0.72))
...
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        self.tensor_patterns = [
            r'\btensor\s+(?:completion|decomposition|rank|contraction|product)\b',
            r'\btucker\s+decomposition\b', r'\bcanonical\s+polyadic\b',
            r'\bcore\s+tensor\b', r'\bfactor\s+matrix\b',
            r'\balternating\s+least\s+squares\b', r'\blow-rank\b',
            r'\bkronecker\s+product\b',
        ]
        self.thermo_patterns = [
            r'\bgibbs\s+free\s+energy\b', r'\bgibbs\s+tensor\b',
            r'\bexcess\s+gibbs\b', r'\benthalpy\s+of\s+mixing\b',
            r'\bentropy\s+of\s+mixing\b', r'\bchemical\s+potential\b',
            r'\bactivity\s+coefficient\b', r'\bheat\s+capacity\b',
            r'\bthermal\s+expansion\b',
        ]
        self.calphad_patterns = [
            r'\bcalphad\b', r'\bredlich-kister\b', r'\bsublattice\s+model\b',
            r'\binteraction\s+parameter\b', r'\bphase\s+diagram\b',
            r'\bscheil-gulliver\b', r'\bmuggianu\b', r'\bgibbs-duhem\b',
        ]
        self.pf_patterns = [
            r'\bphase-field\b', r'\ballen-cahn\b', r'\bcahn-hilliard\b',
            r'\bkks\s+model\b', r'\bgrand\s+potential\b', r'\border\s+parameter\b',
        ]
        self.ai_patterns = [
            r'\bphysics-informed\s+neural\s+network\b', r'\btensor\s+neural\s+network\b',
            r'\bgaussian\s+process\b', r'\bactive\s+learning\b',
            r'\bbayesian\s+optimization\b', r'\buncertainty\s+quantification\b',
        ]
        self.multi_scale_patterns = [
            r'\bdensity\s+functional\s+theory\b', r'\bmolecular\s+dynamics\b',
            r'\bcluster\s+expansion\b', r'\bsqs\b', r'\bab\s+initio\b', r'\bfirst-principles\b',
        ]
        self.all_patterns = (self.tensor_patterns + self.thermo_patterns +
                             self.calphad_patterns + self.pf_patterns +
                             self.ai_patterns + self.multi_scale_patterns)
...
class ReasoningEnhancedGraphBuilder:
        ...
        for rel in self.ontology.relationships:
            if rel.source in valid_concepts and rel.target in valid_concepts:
                if not nx_graph.has_edge(rel.source, rel.target):
                    nx_graph.add_edge(rel.source, rel.target, weight=rel.confidence * 2, cooccurrence=0, semantic=rel.confidence, edge_type=rel.rel_type.value, inferred=True, confidence=rel.confidence)
    ...
    def _compute_final_weights(self, nx_graph: nx.Graph, config: Dict) -> None:
        ...
        for u, v, data in nx_graph.edges(data=True):
            ...
            data['weight'] = cooc_weight * cooc + sem_weight * sem + inf_weight * inf * conf


def build_hybrid_graph(all_concepts: List[List[str]], valid_concepts: List[str], concept_to_id: Dict[str, int],
                       embed_model=None, config: Dict = None, ontology: DomainOntology = None) -> nx.Graph:
    if config is None:
        config = get_adaptive_config(3000)
    nx_graph = nx.Graph()
    for c in valid_concepts:
...
def sample_edges_for_training(nx_graph: nx.Graph, valid_concepts: List[str], concept_to_id: Dict[str, int],
    ...
    target_negs = min(len(pos_pairs) * 2 if pos_pairs else 30, 2000 if memory_safe else 5000)
    attempts = 0
    max_attempts = 50000
    path_lengths = {}
    if not memory_safe:
...
class LatentMoEKGExtractor(nn.Module):
    def __init__(self, num_nodes, num_edge_types, d_model=96, latent_dim=24,
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=num_heads, batch_first=True, dim_feedforward=d_model * 2, dropout=0.1)
    ...
    for concept in valid_concepts:
        ...
        if burst_scores:
            ...
            if max_burst >= burst_threshold:
                burst_data.append({"concept": concept, "burst_score": round(max_burst, 2),
                                   "burst_year": burst_year, "total_mentions": len(concept_years),
...
def detect_semantic_drift(df_filtered: pd.DataFrame, valid_concepts: List[str],
    ...
    for concept in valid_concepts:
        ...
        try:
            ...
            drift_data.append({"concept": concept, "semantic_drift": round(float(drift), 4),
                               "early_papers": len(early_texts), "late_papers": len(late_texts),
...
def build_concept_genealogy(_nx_graph: nx.Graph, valid_concepts: List[str],
    ...
    for concept in valid_concepts:
        ...
        genealogy_data.append({"concept": concept, "pagerank": round(pr, 5), "betweenness": round(bc, 5),
...
def detect_cross_domain_bridges(_nx_graph: nx.Graph, valid_concepts: List[str],
    ...
    for concept in valid_concepts:
        ...
        bridge_data.append({"concept": concept, "bridge_score": round(bridge_score, 4),
    ...
    try:
        ...
        for concept in valid_concepts:
            if concept not in nx_graph:
                continue
            centrality_data.append({"concept": concept, "degree": degree_c.get(concept, 0),
...
def export_publication_figure(nx_graph, valid_concepts, concept_abstract_map,
                              ...
                              filename="tensor_graph_pub.png") -> bytes:
    try:
        pos = nx.spring_layout(nx_graph, seed=42, k=2.5, iterations=200)
        plt.figure(figsize=figsize, dpi=dpi)
        node_colors = [get_tensor_category_color(n) for n in nx_graph.nodes()]
        ...
        plt.title("Thermodynamic Data Tensor (Gibbs Tensor) Concept Graph", fontsize=14, fontweight='bold', pad=20)
        buf = io.BytesIO()
...
def generate_analysis_report(nx_graph, valid_concepts, concept_abstract_map,
                             ...
                             genealogy_df, bridge_df, motifs, val_metrics, df_filtered) -> str:
    report: List[str] = []
    report.append("# Thermodynamic Data Tensor (Gibbs Tensor) Concept Graph Analysis Report")
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by Thermodynamic Data Tensor Concept Graph v6.1*")
    return "\n".join(report)
...
class GraphEditHistory:
    ...
    def save_snapshot(self, nx_graph, valid_concepts, concept_to_id, id_to_concept, concept_abstract_map) -> int:
        snapshot = {
            'id': self._snapshot_counter, 'nx_graph': copy.copy(nx_graph),
            'valid_concepts': list(valid_concepts), 'concept_to_id': dict(concept_to_id),
...
THEME_PRESETS = {
    "Bright (Default)": {
        ...
        "edge_unknown": "rgba(148, 163, 184, 0.30)", "node_border": "#f8fafc",
    ...
    "Dark": {
        ...
        "edge_unknown": "rgba(148, 163, 184, 0.40)", "node_border": "#f8fafc",
    ...
    "Midnight": {
        ...
        "edge_unknown": "rgba(71, 85, 105, 0.40)", "node_border": "#e2e8f0",
        "highlight_bg": "#f43f5e", "hover_bg": "#22d3ee", "shadow_color": "rgba(0,0,0,0.7)",
        "plotly_bg": "#020617", "plotly_paper": "#020617", "grid_color": "#0f172a", "axis_color": "#64748b",
    },
    "Warm": {
        ...
        "edge_unknown": "rgba(120, 53, 15, 0.25)", "node_border": "#fff7ed",
        "highlight_bg": "#dc2626", "hover_bg": "#f59e0b", "shadow_color": "rgba(124, 45, 18, 0.15)",
        "plotly_bg": "#fff7ed", "plotly_paper": "#fff7ed", "grid_color": "#fed7aa", "axis_color": "#9a3412",
    },
    "Forest": {
        ...
        "edge_unknown": "rgba(20, 83, 45, 0.25)", "node_border": "#f0fdf4",
        "highlight_bg": "#15803d", "hover_bg": "#84cc16", "shadow_color": "rgba(20, 83, 45, 0.15)",
        "plotly_bg": "#f0fdf4", "plotly_paper": "#f0fdf4", "grid_color": "#bbf7d0", "axis_color": "#166534",
    },
    "Ocean": {
        ...
        "edge_unknown": "rgba(21, 94, 117, 0.25)", "node_border": "#ecfeff",
        "highlight_bg": "#0ea5e9", "hover_bg": "#22d3ee", "shadow_color": "rgba(8, 51, 68, 0.15)",
        "plotly_bg": "#ecfeff", "plotly_paper": "#ecfeff", "grid_color": "#a5f3fc", "axis_color": "#0e7490",
    },
}
...
# ----------------------------------------------------------------------------
def get_mpea_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    """Backward-compatible alias for get_tensor_category_color."""
    return get_tensor_category_color(concept, cmap_colors)
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
                       ...
                       show_definitions=True, edge_lightness=0.6,
                       edge_color_mode="theme", custom_edge_color="#AAAAAA",
                       tooltip_font_size=13, node_legend_font_size=13) -> None:
...
var options = {{
    "physics": {{
        ...
        "stabilization": {{
            "enabled": true, "iterations": 500, "updateInterval": 50,
            "onlyDynamicEdges": true, "fit": true
        }}
    }},
    "interaction": {{
...
""")
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        color = get_tensor_category_color(node, cmap_colors)
        ...
        if use_abbreviated_labels and len(original_label) > max_label_length:
            ...
            if node_label_position == "top":
                ...
                v_adjust = int(node_label_size) + 4
            font_dict = {'color': theme['font'], 'size': int(node_label_size), 'face': node_font_face, 'strokeWidth': 0, 'vadjust': v_adjust}
        ...
        tooltip_content += "</div>"
        net.add_node(node, label=label, size=size, color={'background': color, 'border': theme['node_border'],
                     ...
                     shadow={'enabled': True, 'color': theme['shadow_color'], 'size': 12, 'x': 4, 'y': 4}, shape=node_shape)
    ...
    for u, v in nx_graph.edges():
        ...
        edge_kwargs = dict(
            ...
            color={'color': base_color, 'highlight': theme['highlight_bg'],
                   'hover': theme['hover_bg'], 'opacity': 0.85},
            smooth={"type": "dynamic"},
            title=(f"<span style='font-family:{node_font_face};'>Weight: <b>{w:.2f}</b><br>"
                   f"Type: {edge_type}<br>Inferred: {is_inferred}</span>"),
            dashes=dashes,
        )
    ...
    if used_rel_types:
        ...
        legend_html = (f'<div style="background:#0d0d1a;border-radius:8px;padding:12px 16px;margin-top:8px;max-height:280px;overflow-y:auto;"><div style="color:#fff;font-size:13px;font-weight:bold;margin-bottom:6px;">Edge Colors ({len(used_rel_types)} relationship types)</div><table style="border-collapse:collapse;">{"".join(legend_rows)}</table></div>')
    ...
    try:
        ...
        if use_abbreviated_labels and label_map:
            ...
            hidden_div = f'<div id="hea-label-map-data" style="display:none;">{label_map_json}</div>'
...
div.vis-network div.vis-manipulation {{ background: {theme['tooltip_bg']} !important; border-top: 1px solid {theme['tooltip_border']} !important; color: {theme['font']} !important; }}
        ...
        if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
            ...
            function formatEdgeRow(e, idx, mode) {
                ...
                return '<div style="padding:8px 10px;margin:4px 0;background:rgba(248,250,252,0.9);border-left:4px solid ' + typeColor + ';border-radius:6px;font-size:12px;">' +
                    ...
                    '</div><div style="display:flex;align-items:center;gap:8px;padding-left:10px;">' +
            ...
            function showEdgeInfoPanel(nodeId, connectedEdges) {
                ...
                panel.style.cssText = 'position:fixed;top:90px;right:20px;width:400px;max-height:calc(100vh - 110px);overflow-y:auto;z-index:9999;background:rgba(255,255,255,0.95);border:1px solid rgba(255,215,0,0.6);border-radius:16px;padding:0;font-family:Inter,Segoe UI,Roboto,sans-serif;box-shadow:0 20px 60px rgba(0,0,0,0.15);backdrop-filter:blur(20px);';
                ...
                html += '</div><div id="edges-container" style="padding:12px 16px 16px;">';
                var edgeList = [];
                ...
                window._heaSetLabelMode = function(mode) {
                    labelMode = mode;
                    var p = document.getElementById('edge-info-panel'); if (!p || !p._edgeList) return;
                    ...
                    var container = document.getElementById('edges-container'); if (container) { var newHtml = ''; p._edgeList.forEach(function(e, idx){ newHtml += formatEdgeRow(e, idx, mode); }); container.innerHTML = newHtml; }
                };
            }
...
<span style='font-size:{node_legend_font_size}px; color:{theme.get('font', '#1e293b')};'>{full}</span>
    ...
    try:
        ...
        st.download_button("Download Interactive Graph (HTML)", data=html_bytes,
                           file_name="tensor_concept_graph.html", mime="text/html")
...
def build_category_hierarchy(concepts: List[str], concept_abstract_map: Dict[str, List[int]],
                             top_n_per_category: int = 0) -> Tuple[List[str], List[str], List[float]]:
    labels: List[str] = []
    parents: List[str] = []
    values: List[float] = []
    ...
    root_label = "Thermodynamic Data Tensor"
...
def render_graph_plotly_2d(nx_graph, concept_abstract_map, cmap_name="viridis", top_n_nodes=0,
                            theme=None, show_edge_weights=False, node_label_size=10) -> None:
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if top_n_nodes > 0 and len(nx_graph.nodes()) > top_n_nodes:
        degrees = dict(nx_graph.degree(weight='weight'))
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        node_color.append(get_tensor_category_color(node, cmap_colors))
    ...
    fig.add_trace(go.Scatter(x=node_x, y=node_y, mode='markers+text',
                             ...
                             textposition='top center', textfont=dict(size=node_label_size, color=theme.get('font', '#000')),
                             hovertext=node_text, hoverinfo='text'))
    fig.update_layout(showlegend=False, paper_bgcolor=theme.get('plotly_paper', '#ffffff'),
                      plot_bgcolor=theme.get('plotly_bg', '#ffffff'), font_color=theme.get('font', '#000000'),
                      xaxis=dict(showgrid=False, zeroline=False, showticklabels=False),
                      yaxis=dict(showgrid=False, zeroline=False, showticklabels=False), title="2D Concept Graph (Plotly)")
    st.plotly_chart(fig, use_container_width=True)
...
def render_graph_plotly_3d(nx_graph, concept_abstract_map, cmap_name="viridis", top_n_nodes=0,
    ...
    fig.add_trace(go.Scatter3d(x=edge_x, y=edge_y, z=edge_z, mode='lines',
                               line=dict(color='rgba(150,150,150,0.3)', width=1), hoverinfo='none'))
    fig.add_trace(go.Scatter3d(x=node_x, y=node_y, z=node_z, mode='markers+text',
                               marker=dict(size=node_size, color=node_color, line=dict(width=1, color='white')),
                               text=[n.replace('_', ' ').title() for n in nx_graph.nodes()],
                               textposition='top center', textfont=dict(size=8, color=theme.get('font', '#000')),
                               hovertext=node_text, hoverinfo='text'))
    fig.update_layout(showlegend=False, paper_bgcolor=theme.get('plotly_paper', '#ffffff'),
                      scene=dict(xaxis=dict(showgrid=False, showticklabels=False, title=''),
                                 yaxis=dict(showgrid=False, showticklabels=False, title=''),
                                 zaxis=dict(showgrid=False, showticklabels=False, title='')),
                      title="3D Concept Graph (Plotly)", margin=dict(l=0, r=0, b=0, t=40))
    st.plotly_chart(fig, use_container_width=True)
...
def render_graph_fallback(nx_graph, concept_abstract_map, theme=None, show_edge_weights=False) -> None:
    ...
    top_nodes = sorted(nx_graph.nodes(), key=lambda n: len(concept_abstract_map.get(n, [])), reverse=True)[:20]
    node_data = []
    for node in top_nodes:
        freq = len(concept_abstract_map.get(node, []))
        degree = nx_graph.degree(node)
        node_data.append({'Concept': node.replace('_', ' ').title(), 'Frequency': freq,
                          'Degree': degree, 'Category': abstract_concepts_to_categories([node]).get(node, 'general')})
...
def render_radar_chart(distill_df, top_k=15, cmap_name="viridis", theme=None,
                       label_size=12, font_size=11) -> None:
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if distill_df.empty:
    ...
    fig = go.Figure()
    metrics_to_plot = [('frequency', 'Frequency'), ('semantic_density', 'Semantic Density'), ('coherence_score', 'Coherence')]
    for i, (metric, name) in enumerate(metrics_to_plot):
        if metric in df.columns:
            values = df[metric].tolist()
            values += values[:1]
            cat_loop = categories + categories[:1]
            fig.add_trace(go.Scatterpolar(r=values, theta=cat_loop, fill='toself', name=name,
                                          line=dict(color=cmap_colors[i], width=2)))
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 1.1], tickfont=dict(size=int(font_size), color=theme.get('axis_color', '#000'))),
                                  angularaxis=dict(tickfont=dict(size=int(label_size), color=theme.get('font', '#000')))),
                      showlegend=True, title=f"Concept Distillation Radar (Top {min(top_k, 10)})",
                      paper_bgcolor=theme.get('plotly_paper', '#ffffff'),
...
def render_tsne_projection(valid_concepts, concept_abstract_map, embed_model, theme=None) -> None:
    if theme is None:
        theme = THEME_PRESETS["Bright (Default)"]
    if len(valid_concepts) < 5:
    ...
    try:
        ...
        for node in valid_concepts:
            if node in node_community:
                comm_data.append({'Concept': node.replace('_', ' ').title(), 'Community': node_community[node],
                                  'Frequency': len(concept_abstract_map.get(node, [])), 'Degree': nx_graph.degree(node)})
        if comm_data:
            comm_df = pd.DataFrame(comm_data)
            st.dataframe(comm_df, use_container_width=True)
            fig = px.scatter(comm_df, x='Degree', y='Frequency', color='Community', hover_data=['Concept'],
...
def render_concept_growth(df_filtered, valid_concepts, concept_abstract_map, theme=None) -> None:
    ...
    if growth_data:
        ...
        fig = px.line(growth_df, x="Year", y="Count", color="Concept", title="Concept Growth Over Time",
                      template="plotly_white" if theme == THEME_PRESETS["Bright (Default)"] else "plotly_dark")
        fig.update_layout(paper_bgcolor=theme.get('plotly_paper', '#ffffff'),
                          plot_bgcolor=theme.get('plotly_bg', '#ffffff'), font_color=theme.get('font', '#000000'))
        st.plotly_chart(fig, use_container_width=True)
...
def render_bubble_chart(nx_graph, valid_concepts, concept_abstract_map, distill_df, theme=None) -> None:
    ...
    for concept in valid_concepts:
        ...
        bubble_data.append({'Concept': concept.replace('_', ' ').title(), 'Frequency': freq,
                            'Degree': degree, 'Category': category, 'Efficiency': efficiency})
    if bubble_data:
        bubble_df = pd.DataFrame(bubble_data)
        fig = px.scatter(bubble_df, x='Degree', y='Frequency', size='Efficiency', color='Category',
                         hover_name='Concept', title="Concept Importance Bubble Chart", size_max=50,
                         template="plotly_white" if theme == THEME_PRESETS["Bright (Default)"] else "plotly_dark")
        fig.update_layout(paper_bgcolor=theme.get('plotly_paper', '#ffffff'),
                          plot_bgcolor=theme.get('plotly_bg', '#ffffff'), font_color=theme.get('font', '#000000'))
        st.plotly_chart(fig, use_container_width=True)
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis",
    ...
    if len(labels) != len(parents) or len(labels) != len(values):
        st.error(f"Sunburst data mismatch.")
        return
    if theme is None:
    ...
    try:
        ...
        try:
            ...
            qual_palettes = [px_colors.qualitative.Bold, px_colors.qualitative.Vivid, px_colors.qualitative.Safe, px_colors.qualitative.Pastel, px_colors.qualitative.Dark24, px_colors.qualitative.Light24]
    ...
    try:
        fig = go.Figure(go.Sunburst(
            ids=unique_ids, labels=display_labels, parents=parent_ids, values=values,
            customdata=labels, branchvalues=bv,
            marker=dict(colors=sunburst_colors, line=dict(width=0.5, color="rgba(255,255,255,0.25)")),
            textinfo=textinfo,
            hovertemplate=('<b>%{customdata}</b><br>Value: %{value}<br>Symbol: %{label}<extra></extra>' if hover_info == "all" else '<b>%{customdata}</b><extra></extra>' if hover_info == "minimal" else '<extra></extra>'),
            insidetextorientation="radial", textfont=dict(size=int(label_size), family=font_family, color="white"),
        ))
    except Exception as e:
    ...
    fig.update_layout(margin=dict(l=0, r=0, t=80, b=0),
                      ...
                      title=dict(text="<b>Hierarchical Concept Map</b><br><sup>★ Parent | ★□ Child | ★□◆ Grandchild — Hover for names</sup>", font=dict(size=16, family=font_family)))
    st.plotly_chart(fig, use_container_width=True)
...
def render_concept_timeline(df_filtered, valid_concepts, concept_abstract_map, theme=None) -> None:
    ...
    fig = px.line(timeline_df, x="Year", y="Count", color="Concept",
                  title="Concept Frequency Over Time", labels={"Count": "Mentions", "Year": "Publication Year"},
                  template="plotly_white" if theme == THEME_PRESETS["Bright (Default)"] else "plotly_dark")
    fig.update_layout(paper_bgcolor=theme.get('plotly_paper', '#ffffff'),
                      plot_bgcolor=theme.get('plotly_bg', '#ffffff'), font_color=theme.get('font', '#000000'))
    st.plotly_chart(fig, use_container_width=True)
...
def export_graph(nx_graph, concept_abstract_map, export_format: str, include_metadata: bool = True
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'ThermodynamicTensor-ConceptGraph'
            try:
                nx.write_graphml_lxml(nx_graph, "tensor_graph.graphml")
            except Exception:
                nx.write_graphml(nx_graph, "tensor_graph.graphml")
            with open("tensor_graph.graphml", "rb") as f:
                return f.read(), "application/graphml+xml", "tensor_graph.graphml"
        except Exception as e:
        ...
        if include_metadata:
            data['metadata'] = {'created': datetime.now().isoformat(), 'version': '6.1',
                                'tool': 'ThermodynamicTensor-ConceptGraph',
                                'node_count': len(nx_graph.nodes()), 'edge_count': len(nx_graph.edges())}
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "tensor_graph_full.json"
    elif export_format == "JSON (Compact)":
        data = nx.node_link_data(nx_graph)
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "tensor_graph.json"
    elif export_format == "CSV (Edges + Metadata)":
        edge_data = []
        for u, v, data in nx_graph.edges(data=True):
            edge_data.append({"source": u, "target": v, "weight": data.get('weight', 1),
                              "cooccurrence": data.get('cooccurrence', 0), "semantic_similarity": data.get('semantic', 0),
                              "edge_type": data.get('edge_type', 'unknown'), "inferred": data.get('inferred', False),
                              "confidence": data.get('confidence', 1.0), "path": data.get('path', '')})
        csv_df = pd.DataFrame(edge_data)
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "tensor_edges_enhanced.csv"
    elif export_format == "CSV (Nodes + Metadata)":
        node_data = []
        for node in nx_graph.nodes():
            row = {"concept": node, "frequency": len(concept_abstract_map.get(node, [])),
                   "degree": nx_graph.degree(node), "concept_type": nx_graph.nodes[node].get('concept_type', 'general'),
                   "definition": nx_graph.nodes[node].get('definition', ''),
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "tensor_nodes_enhanced.csv"
        ...
        try:
            ...
            return buf.read(), "image/png", "tensor_graph.png"
        except Exception as e:
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '6.1'
            nx.write_gexf(nx_graph, "tensor_graph.gexf")
            with open("tensor_graph.gexf", "rb") as f:
                return f.read(), "application/xml", "tensor_graph.gexf"
        except Exception as e:
...
def render_reasoning_dashboard(nx_graph, valid_concepts, ontology, extractor) -> None:
    ...
    if rel_types:
        ...
        st.plotly_chart(fig, use_container_width=True)
    st.subheader("🔗 Inferred Tensor-Thermodynamic Chains")
    method_nodes = [c for c in valid_concepts if c in ontology.concepts and ontology.concepts[c].concept_type == ConceptType.METHOD]
    ...
    for proc in method_nodes[:5]:
        for prop in property_nodes[:5]:
            paths = ontology.infer_path(proc, prop, max_depth=3)
            if paths:
                chains_found.append({"Method": proc, "Property": prop, "Path Length": len(paths[0]), "Path": " → ".join(paths[0])})
    if chains_found:
    ...
    synonym_examples = [
        ...
        ("pinn", "physics_informed_neural_network"),
        ("calphad", "calphad"),
    ]
...
def render_microtransformer_kg_rag_tab(analysis_data: Dict, ontology: DomainOntology):
    ...
    quick_query = st.text_input(
        ...
        value="How does tensor completion affect gibbs free energy?",
        key="mt_quick_query",
        placeholder="e.g., How does tensor rank affect excess gibbs energy?",
    )
    nlp_src, nlp_tgt = None, None
    if quick_query.strip():
        q_lower = quick_query.lower()
        if "tensor completion" in q_lower and "gibbs" in q_lower:
            nlp_src = "tensor_completion"
            nlp_tgt = "gibbs_free_energy"
        elif "tensor rank" in q_lower and "gibbs" in q_lower:
            nlp_src = "tensor_rank"
            nlp_tgt = "gibbs_free_energy"
        elif "calphad" in q_lower and "phase" in q_lower:
            nlp_src = "calphad"
            nlp_tgt = "phase_stability"
...
def extract_doc_metrics(text: str) -> Dict[str, Any]:
    ...
    if temp_matches:
        metrics['temperature'] = [float(m) for m in temp_matches]
    energy_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:j|kj|mj|ev)', text, re.I)
    if energy_matches:
        metrics['energy'] = [float(m) for m in energy_matches]
    pressure_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mpa|gpa|pa|kpa)', text, re.I)
    if pressure_matches:
        metrics['pressure'] = [float(m) for m in pressure_matches]
    return metrics
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    example_queries = [q for pdef in TENSOR_PROBLEM_DEFINITIONS.values() for q in pdef.example_queries[:1]]
    selected_example = st.sidebar.selectbox("Or select an example:", [""] + example_queries, key="example_query_select")
    query = st.sidebar.text_area("Your thermodynamic tensor question:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How does tensor rank affect Gibbs energy prediction?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    st.markdown("Ask a specific scientific question about thermodynamic data tensors. The system will dynamically expand the ontology, extract a relevant subgraph, and generate a grounded answer using retrieved literature snippets.")
    ...
    with col1:
        query = st.text_input("Enter your research question:", placeholder="e.g., How does tensor completion improve Gibbs energy prediction?")
    with col2:
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.subheader("🔬 Thermodynamic Data Tensor Focus Areas")
        st.markdown(r"- **Tensor Methods:** Rank, Tucker/CP decomposition, Tensor Completion, ALS, Kronecker Product")
        st.markdown(r"- **Thermodynamic Properties:** Gibbs Free Energy, Excess Gibbs, Enthalpy/Entropy of mixing, Chemical Potential, Activity Coefficient")
        st.markdown(r"- **CALPHAD Methods:** Redlich-Kister, Sublattice Model, Phase Diagrams, Scheil-Gulliver, Muggianu extrapolation")
        st.markdown("- **Phase-Field & Microstructure:** Allen-Cahn, Cahn-Hilliard, KKS, Grand Potential")
        st.markdown("- **AI & Data-Driven:** PINNs, Tensor Neural Networks, GPR, Active Learning, Bayesian Optimization, UQ")
        st.markdown("- **Multi-Scale Methods:** DFT, MD, Cluster Expansion, SQS")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox(
            "Use ontology-based resolution", value=True,
            help="Maps synonyms like 'Gibbs energy' to canonical concept 'gibbs_free_energy'",
        )
        ...
        st.session_state['use_relationship_extraction'] = st.checkbox(
            ...
            help="Identifies causal links between tensor methods and thermodynamic properties",
        )
        st.session_state['use_inference'] = st.checkbox(
            "Enable reasoning-based edge inference", value=True,
            help="Infers tensor -> thermo -> stability chains even when not co-occurring",
        )
...
# ============================================================================
def main() -> None:
    st.title("🔬 Thermodynamic Data Tensor (Gibbs Tensor) Concept Graph v6.1")
    st.caption(
        "Multi-level reasoning concept graph for numerical/quantitative description of Thermodynamic Data Tensors | "
        "Focus: Tensor Decomposition, CALPHAD, Phase-Field, and AI Surrogates for Gibbs Energy | "
        ...
        try:
            with status:
                ...
                def _process_single_row(idx, row):
                    text = " ".join([str(row[col]) for col in selected_text_cols if col in row and pd.notna(row[col])])
                    if use_ontology and extractor is not None:
                        concepts = extractor.extract_from_text(text, idx, allowed_concepts=whitelist)
                    else:
                        concepts = extract_concepts_from_text(text)
                    ...
                    energy_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:j|kj|mj|ev)', text, re.I)
                    if energy_matches: doc_metrics['energy'] = [float(m) for m in energy_matches]
                    pressure_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mpa|gpa|pa|kpa)', text, re.I)
                    if pressure_matches: doc_metrics['pressure'] = [float(m) for m in pressure_matches]
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button("📄 Download Report", data=report.encode('utf-8'), file_name="tensor_report.md", mime="text/markdown")
