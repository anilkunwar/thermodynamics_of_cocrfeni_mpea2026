...
==========================================================================
Multi-level reasoning concept graph for high-dimensional thermodynamic data tensors,
with focus on Gibbs free energy as a function of composition, temperature, and pressure.
Architecture ported from CoCrFeNi MPEA Quantitative Descriptor Graph.
"""
...
# ============================================================================
st.set_page_config(
    page_title="Thermodynamic Data Tensor Concept Graph v6.1",
    page_icon="📊",
    layout="wide",
...
# ============================================================================
# ENHANCED ONTOLOGY & NLP REASONING SYSTEM (THERMODYNAMIC TENSOR DOMAIN)
# ============================================================================
...
# 32 Specialized Latent Experts for Thermodynamic Tensors
TENSOR_EXPERT_LABELS = [
    "Tensor Completion", "Tucker Decomposition", "CP Decomposition", "ALS Algorithm",
    "Low-Rank Approximation", "Kronecker Product", "Tensor Contraction", "Core Tensor",
    "Factor Matrix", "Gibbs Free Energy", "Excess Gibbs", "Enthalpy of Mixing",
    "Entropy of Mixing", "Chemical Potential", "Activity Coefficient", "CALPHAD",
    "Redlich-Kister", "Sublattice Model", "Interaction Parameter", "Phase Diagram",
    "Phase-Field Model", "Allen-Cahn", "Cahn-Hilliard", "KKS Model",
    "PINN", "Tensor Neural Network", "Gaussian Process", "Active Learning",
    "Bayesian Optimization", "Uncertainty Quantification", "DFT", "Molecular Dynamics"
]
...
def render_sankey_diagram(token_labels: List[str], routing_np: np.ndarray,
    ...
    """
    Render a Sankey diagram showing token → expert flow.
    Only top-K experts per token are shown to avoid clutter.
    """
    n_tokens, n_experts = routing_np.shape

    # Labels: tokens + experts
    token_names = [f"Token: {t}" for t in token_labels]
    expert_names = TENSOR_EXPERT_LABELS
    all_labels = token_names + expert_names

    sources, targets, values = [], [], []
    for i in range(n_tokens):
        weights = routing_np[i]
        top_indices = np.argsort(weights)[-top_k_flow:][::-1]
    ...
    # Node colors: tokens get a qualitative palette, experts use the colormap
    token_colors = px.colors.qualitative.Pastel[:n_tokens]
    expert_colors = scale[:n_experts] if len(scale) >= n_experts else scale * (n_experts // len(scale) + 1)
    node_colors = token_colors + expert_colors

    # Link colors (semi-transparent to match source)
    link_colors = []
    for s in sources:
    ...
    fig.update_layout(
        title_text="Token → Expert Routing (Top {} Experts per Token)".format(top_k_flow),
        height=500,
...
def render_chord_diagram(token_labels: List[str], routing_np: np.ndarray,
    ...
    """
    Render a chord-like circular diagram using Plotly.
    Falls back gracefully if the 'chord' library is unavailable.
    """
    try:
        import chord
        # chord library produces its own HTML; wrap in a Plotly-like figure for consistency
        n_tokens, n_experts = routing_np.shape
        # Build a matrix: tokens × experts (top 5 per token)
        matrix = np.zeros((n_tokens, n_experts))
        for i in range(n_tokens):
            top_idx = np.argsort(routing_np[i])[-5:][::-1]
            matrix[i, top_idx] = routing_np[i, top_idx]
        # chord expects a square matrix, so we pad or use a different approach
        # For simplicity, we use Plotly's circular scatter as a chord proxy
        fig = _render_chord_proxy(token_labels, routing_np, scale, theme)
        return fig
    except ImportError:
        # Fallback to the proxy chord renderer
        return _render_chord_proxy(token_labels, routing_np, scale, theme)


def _render_chord_proxy(token_labels: List[str], routing_np: np.ndarray,
                        scale: List[str], theme: Dict) -> go.Figure:
    """
    Plotly-based circular (chord-like) diagram.
    Tokens and experts are placed on a circle; weighted edges drawn as curved lines.
    """
    n_tokens, n_experts = routing_np.shape
    n_nodes = n_tokens + n_experts
    labels = [f"T: {t}" for t in token_labels] + TENSOR_EXPERT_LABELS
    ...
    # Draw edges (top 3 per token)
    for i in range(n_tokens):
        top_idx = np.argsort(routing_np[i])[-3:][::-1]
        for j in top_idx:
            val = routing_np[i, j]
            if val < 1e-6:
                continue
            src_x, src_y = x[i], y[i]
            tgt_x, tgt_y = x[n_tokens + j], y[n_tokens + j]
            # Quadratic Bezier midpoint
            mid_x = (src_x + tgt_x) / 2 * 0.3
            mid_y = (src_y + tgt_y) / 2 * 0.3
            t = np.linspace(0, 1, 30)
            bx = (1 - t) ** 2 * src_x + 2 * (1 - t) * t * mid_x + t ** 2 * tgt_x
            by = (1 - t) ** 2 * src_y + 2 * (1 - t) * t * mid_y + t ** 2 * tgt_y
            fig.add_trace(go.Scatter(
                x=bx, y=by, mode='lines',
                line=dict(color=scale[j % len(scale)], width=max(1, val * 5)),
                hoverinfo='text',
                text=f"{token_labels[i]} → {TENSOR_EXPERT_LABELS[j]}: {val:.3f}",
                showlegend=False
            ))

    # Draw nodes
    node_colors = px.colors.qualitative.Pastel[:n_tokens] + scale[:n_experts]
    fig.add_trace(go.Scatter(
        x=x, y=y, mode='markers+text',
        marker=dict(size=12, color=node_colors, line=dict(width=1, color='white')),
        text=labels,
        textposition='top center',
        textfont=dict(size=9, color=theme.get("font", "#000")),
        hoverinfo='text',
        showlegend=False
    ))

    fig.update_layout(
        title="Circular Token–Expert Routing (Chord Proxy)",
...
class DomainOntology:
    """Comprehensive ontology for Thermodynamic Data Tensors."""
    ...
    def _build_ontology(self) -> None:
        ...
        # === THERMODYNAMIC QUANTITIES (Composition & Temperature dependent) ===
        ...
        # === PHASE‑FIELD & MICROSTRUCTURE (for tensor coupling) ===
        ...
        self._add_concept("laser_powder_bed_fusion", ConceptType.PROCESS,
            synonyms={"lpbf", "slm", "selective laser melting", "powder bed fusion"},
            definition="Additive manufacturing process using a laser to melt powder beds, characterized by rapid cooling")
        self._add_concept("directed_energy_deposition", ConceptType.PROCESS,
            synonyms={"ded", "laser cladding"},
            definition="Additive manufacturing process where material is injected and melted simultaneously")
        self._add_concept("melt_pool", ConceptType.PHENOMENON,
            synonyms={"weld pool", "fusion zone"},
            definition="Localized molten region in AM characterized by complex fluid flow and high thermal gradients")
        self._add_concept("thermal_gradient", ConceptType.PARAMETER,
            synonyms={"temperature gradient", "g_t"},
            definition="Spatial rate of temperature change in the melt pool, driving microstructure formation")
        self._add_concept("cooling_rate", ConceptType.PARAMETER,
            synonyms={"solidification rate", "quench rate"},
            definition="Temporal rate of temperature decrease, determining phase selection and defect formation")

        # === NON-EQUILIBRIUM & KINETIC THERMODYNAMICS ===
        self._add_concept("non_equilibrium_solidification", ConceptType.PHENOMENON,
            synonyms={"rapid solidification", "non-equilibrium freezing"},
            definition="Solidification occurring too fast for diffusion to maintain equilibrium compositions")
        self._add_concept("solute_trapping", ConceptType.PHENOMENON,
            synonyms={"solute drag", "kinetic suppression"},
            definition="Phenomenon where solute atoms are incorporated into the solid faster than their diffusive speed")
        self._add_concept("path_dependent_driving_force", ConceptType.PROPERTY,
            synonyms={"history dependent driving force", "thermal history driving force"},
            definition="Thermodynamic driving force as a function of spatiotemporal thermal history, not just current state")
        self._add_concept("kinetic_driving_force", ConceptType.PROPERTY,
            synonyms={"kinetic potential", "interface driving pressure"},
            definition="Net driving force at the solid-liquid interface including thermodynamic and kinetic terms")
        self._add_concept("metastable_phase", ConceptType.MICROSTRUCTURE,
            synonyms={"metastable state", "non-equilibrium phase"},
            definition="Phase that is stable under rapid cooling but not at global equilibrium conditions")

        # === CURSE OF DIMENSIONALITY & TENSOR GEOMETRY ===
        self._add_concept("high_dimensional_composition_space", ConceptType.PARAMETER,
            synonyms={"hyper-simplex", "n-dimensional simplex", "composition manifold"},
            definition="The multi-dimensional space of MPEA compositions where data sparsity increases exponentially")
        self._add_concept("gibbs_hessian", ConceptType.PROPERTY,
            synonyms={"hessian matrix", "curvature of gibbs energy"},
            definition="Second derivative of Gibbs energy w.r.t composition, dictating spinodal and stability limits in high-dim")
        self._add_concept("convex_hull_thermodynamics", ConceptType.METHOD,
            synonyms={"tangent plane search", "convex hull calculation"},
            definition="Geometric method to find equilibrium phases in high-dimensional composition space, bottlenecked by CoD")
        self._add_concept("sparse_data_extrapolation", ConceptType.METHOD,
            synonyms={"low-rank tensor extrapolation", "data imputation"},
            definition="Using tensor completion to extrapolate CALPHAD data into uncharted high-dimensional regions")

        # === AM SPECIFIC MICROSTRUCTURES ===
        self._add_concept("cellular_dendritic_structure", ConceptType.MICROSTRUCTURE,
            synonyms={"cellular structure", "dendritic microstructure"},
            definition="Fine microstructure formed in AM due to high thermal gradients and restricted growth directions")
    ...
    def _build_causal_chains(self) -> None:
        causal_chains = [
            ...
            ("phase_field_model", RelationshipType.RESULTS_IN, "microstructure", 0.90),
            ...
            ("compositional_descriptor", RelationshipType.INFLUENCES, "phase_diagram", 0.80),
                    # Additive Manufacturing -> Thermal Conditions
            ("laser_powder_bed_fusion", RelationshipType.CAUSES, "melt_pool", 0.95),
            ("laser_powder_bed_fusion", RelationshipType.CAUSES, "non_equilibrium_solidification", 0.90),
            ("directed_energy_deposition", RelationshipType.CAUSES, "melt_pool", 0.85),

            # Thermal Conditions -> Non-Equilibrium Thermodynamics
            ("cooling_rate", RelationshipType.DRIVES, "non_equilibrium_solidification", 0.95),
            ("thermal_gradient", RelationshipType.INFLUENCES, "cellular_dendritic_structure", 0.85),
            ("non_equilibrium_solidification", RelationshipType.CAUSES, "solute_trapping", 0.90),
            ("solute_trapping", RelationshipType.MODIFIES, "chemical_potential", 0.85),

            # Thermal History -> Driving Force
            ("cooling_rate", RelationshipType.INFLUENCES, "path_dependent_driving_force", 0.90),
            ("path_dependent_driving_force", RelationshipType.DEPENDS_ON, "gibbs_tensor", 0.85),
            ("kinetic_driving_force", RelationshipType.COMPUTES, "non_equilibrium_solidification", 0.80),

            # Curse of Dimensionality Context
            ("high_dimensional_composition_space", RelationshipType.CONSTRAINS, "gibbs_tensor", 0.90),
            ("high_dimensional_composition_space", RelationshipType.DRIVES, "sparse_data_extrapolation", 0.85),
            ("gibbs_hessian", RelationshipType.BOUNDS, "convex_hull_thermodynamics", 0.85),
            ("sparse_data_extrapolation", RelationshipType.ENABLES, "convex_hull_thermodynamics", 0.75),

            # Microstructure Evolution
            ("path_dependent_driving_force", RelationshipType.RESULTS_IN, "metastable_phase", 0.80),
            ("kinetic_driving_force", RelationshipType.RESULTS_IN, "cellular_dendritic_structure", 0.75),
]
...
_HIERARCHY_PARENTS: Dict[str, Tuple[str, int]] = {
    ...
    "laser_powder_bed_fusion": ("AM Processes", 1),
    "directed_energy_deposition": ("AM Processes", 1),
    "melt_pool": ("AM Thermal Phenomena", 1),
    "thermal_gradient": ("AM Thermal Phenomena", 1),
    "cooling_rate": ("AM Thermal Phenomena", 1),

    "non_equilibrium_solidification": ("Non-Equilibrium Thermodynamics", 1),
    "solute_trapping": ("Non-Equilibrium Thermodynamics", 1),
    "path_dependent_driving_force": ("Non-Equilibrium Thermodynamics", 1),
    "kinetic_driving_force": ("Non-Equilibrium Thermodynamics", 1),
    "metastable_phase": ("Non-Equilibrium Thermodynamics", 1),

    "high_dimensional_composition_space": ("Curse of Dimensionality", 1),
    "gibbs_hessian": ("Curse of Dimensionality", 1),
    "convex_hull_thermodynamics": ("Curse of Dimensionality", 1),
    "sparse_data_extrapolation": ("Curse of Dimensionality", 1),

    "cellular_dendritic_structure": ("AM Microstructures", 1),
...
# ============================================================================
# THERMODYNAMIC TENSOR KEYWORDS & PATTERNS
# ============================================================================
TENSOR_KEYWORDS = [
    ...
    "solute drag",
    "kinetic suppression",
    "path dependent driving force",
    "kinetic driving force",
    "metastable phase",
    "high dimensional composition space",
    "hyper-simplex",
    "composition manifold",
    "gibbs hessian",
    "hessian matrix",
    "convex hull thermodynamics",
    "tangent plane search",
    "sparse data extrapolation",
...
TENSOR_QUANTITATIVE_PATTERNS = [
    ...
    r'\bcooling\s+rate\b',
    r'\bnon-equilibrium\s+solidification\b',
    r'\brapid\s+solidification\b',
    r'\bsolute\s+trapping\b',
    r'\bsolute\s+drag\b',
    r'\bpath\s+dependent\s+driving\s+force\b',
    r'\bkinetic\s+driving\s+force\b',
    r'\bmetastable\s+phase\b',
    r'\bhigh\s+dimensional\s+composition\s+space\b',
    r'\bgibbs\s+hessian\b',
    r'\bconvex\s+hull\s+thermodynamics\b',
    r'\bsparse\s+data\s+extrapolation\b',
    r'\bcellular\s+dendritic\s+structure\b',
...
def normalize_tensor_concept(concept: str) -> str:
    ...
    concept = re.sub(r'laser\s+powder\s+bed\s+fusion', 'laser_powder_bed_fusion', concept)
    concept = re.sub(r'lpbf', 'laser_powder_bed_fusion', concept)
    concept = re.sub(r'directed\s+energy\s+deposition', 'directed_energy_deposition', concept)
    concept = re.sub(r'melt\s+pool', 'melt_pool', concept)
    concept = re.sub(r'thermal\s+gradient', 'thermal_gradient', concept)
    concept = re.sub(r'cooling\s+rate', 'cooling_rate', concept)
    concept = re.sub(r'non-equilibrium\s+solidification', 'non_equilibrium_solidification', concept)
    concept = re.sub(r'solute\s+trapping', 'solute_trapping', concept)
    concept = re.sub(r'path\s+dependent\s+driving\s+force', 'path_dependent_driving_force', concept)
    concept = re.sub(r'kinetic\s+driving\s+force', 'kinetic_driving_force', concept)
    concept = re.sub(r'metastable\s+phase', 'metastable_phase', concept)
    concept = re.sub(r'high\s+dimensional\s+composition\s+space', 'high_dimensional_composition_space', concept)
    concept = re.sub(r'gibbs\s+hessian', 'gibbs_hessian', concept)
    concept = re.sub(r'convex\s+hull\s+thermodynamics', 'convex_hull_thermodynamics', concept)
    concept = re.sub(r'sparse\s+data\s+extrapolation', 'sparse_data_extrapolation', concept)
    concept = re.sub(r'cellular\s+dendritic\s+structure', 'cellular_dendritic_structure', concept)
    return concept
...
def get_tensor_category_color(concept: str, cmap_colors=None) -> str:
    ...
    color_map = {
        ...
        'general': '#c5b0d5',
        'am_process': '#FF6B9D',
        'am_thermal_phenomena': '#C44569',
        'non_equilibrium_thermodynamics': '#786FA6',
        'curse_of_dimensionality': '#F8B500',
        'am_microstructure': '#574B90',
...
class AdvancedConceptResolver:
    ...
    def _context_disambiguation(
        ...
        if 'tensor' in text:
            if 'decomposition' in context_lower:
                return 'tucker_decomposition'
            elif 'completion' in context_lower:
                return 'tensor_completion'
            elif 'rank' in context_lower:
                return 'tensor_rank'
        if 'gibbs' in text:
            if 'excess' in context_lower:
                return 'excess_gibbs_energy'
            else:
                return 'gibbs_free_energy'
        if 'phase' in text:
            if 'diagram' in context_lower:
                return 'phase_diagram'
            elif 'stability' in context_lower:
                return 'phase_stability'
        return None


# ============================================================================
# ENHANCED CONCEPT EXTRACTOR (Tensor domain)
# ============================================================================
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self) -> None:
        self.tensor_patterns = [
            ...
            r'\bsublattice\s+model\b',
            r'\binteraction\s+parameter\b',
            r'\bphase\s+diagram\b',
            r'\bphase\s+stability\b',
            r'\bphase-field\s+model\b',
            r'\ballen-cahn\b',
            r'\bcahn-hilliard\b',
            r'\bkks\s+model\b',
            r'\bgrand\s+potential\b',
        ...
        self.compiled_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.tensor_patterns
        ]
...
def detect_cross_domain_bridges(
    ...
    if _nx_graph.number_of_nodes() < 5:
        return pd.DataFrame()
    category_map = {c: categorize_tensor_concept(c) for c in valid_concepts}
    try:
        betweenness = nx.betweenness_centrality(_nx_graph, weight='weight')
    except Exception:
        betweenness = {n: 0.0 for n in _nx_graph.nodes()}
...
def export_publication_figure(
    ...
    try:
        ...
        plt.title(
            "Thermodynamic Data Tensor Concept Graph",
...
def generate_analysis_report(
    ...
    report: List[str] = []
    report.append("# Thermodynamic Data Tensor Concept Graph Analysis Report")
    report.append(
...
<span style='font-size:{node_legend_font_size}px; color:{theme.get('font', '#1e293b')};'>{full}</span>
    ...
    try:
        ...
        st.download_button(
            ...
            data=html_bytes,
            file_name="tensor_concept_graph.html",
            mime="text/html",
        )
...
def build_category_hierarchy(
    ...
    category_map = {c: categorize_tensor_concept(c) for c in concepts}
    ...
    root_label = "Thermodynamic Tensors"
...
def render_graph_fallback(
    ...
    for node in top_nodes:
        ...
        node_data.append({
            ...
            'Frequency': freq,
            'Degree': degree,
            'Category': categorize_tensor_concept(node),
        })
...
def render_tsne_projection(
    ...
    try:
        ...
        categories = [categorize_tensor_concept(c) for c in valid_concepts]
...
def render_bubble_chart(
    ...
    for concept in valid_concepts:
        ...
        category = categorize_tensor_concept(concept)
    ...
    if export_format == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'Tensor-ConceptGraph'
            try:
        ...
        if include_metadata:
            data['metadata'] = {
                ...
                'categories': list(set(
                    categorize_tensor_concept(c) for c in nx_graph.nodes()
                )),
            }
        ...
        try:
            ...
            node_colors = [
                get_tensor_category_color(n) for n in nx_graph.nodes()
            ]
            nx.draw(
        ...
        try:
            ...
            buf.seek(0)
            plt.close()
            return buf.read(), "image/svg+xml", "tensor_graph.svg"
        except Exception as e:
...
def render_reasoning_dashboard(
    ...
    synonym_examples = [
        ...
        ("tensor completion", "tensor_completion"),
        ("calphad", "calphad"),
...
def render_microtransformer_kg_rag_tab(analysis_data: Dict, ontology: DomainOntology):
    ...
    if run:
        ...
        with c_flow2:
            top_k_flow = st.slider(
                "Top-K experts per token",
                min_value=1,
                max_value=8,
                value=st.session_state.get("mt_flow_top_k", 3),
                key="mt_flow_top_k"
            )
        if flow_type == "Sankey":
            fig_flow = render_sankey_diagram(token_labels, routing_np, scale, theme, top_k_flow)
        else:
            try:
                fig_flow = render_chord_diagram(token_labels, routing_np, scale, theme)
            except Exception as e:
                st.warning(f"Chord rendering failed ({e}). Falling back to Sankey.")
                fig_flow = render_sankey_diagram(token_labels, routing_np, scale, theme, top_k_flow)
    ...
    def _process_one_batch(batch_num: int) -> None:
        ...
        for local_i, (_, row) in enumerate(batch_df.iterrows()):
            ...
            if use_ontology and extractor is not None:
                ...
                concepts = extract_tensor_concepts_from_text(text)
...
TENSOR_PROBLEM_DEFINITIONS: Dict[TensorThermodynamicsProblem, TensorProblemDefinition] = {
    ...
    TensorThermodynamicsProblem.GIBBS_ENERGY_PREDICTION: TensorProblemDefinition(
        ...
        example_queries=["How to predict Gibbs energy for arbitrary compositions?",
    ...
    TensorThermodynamicsProblem.UNCERTAINTY_ANALYSIS: TensorProblemDefinition(
        ...
        key_concepts=["uncertainty_quantification", "gaussian_process_regression", "epistemic_uncertainty", "aleatoric_uncertainty"],
    ...
    TensorThermodynamicsProblem.CALPHAD_OPTIMIZATION: TensorProblemDefinition(
        ...
        scientific_description="Use tensor completion and decomposition to accelerate CALPHAD database construction and parameter optimisation.",
        root_cause="CALPHAD assessments are time‑consuming and require expert knowledge.",
        key_concepts=["calphad", "tensor_completion", "interaction_parameter", "redlich_kister_polynomials", "sublattice_model"],
        key_relationships=[("tensor_completion", "INFLUENCES", "calphad"),
                           ("redlich_kister_polynomials", "INFLUENCES", "calphad")],
        solution_directions=["Tensor‑assisted parameter fitting", "Active learning for key experiments", "Bayesian optimisation of interaction parameters"],
        relevant_descriptors=["interaction_parameter", "excess_gibbs_energy"],
        relevant_phenomena=["phase_stability"],
        relevant_properties=["gibbs_free_energy"],
        example_queries=["How can tensor completion improve CALPHAD database quality?",
                         "Optimise Redlich‑Kister coefficients using tensor regression."],
        visualization_focus=["parameter_landscape", "convergence_plot"]
    ),
    TensorThermodynamicsProblem.MULTI_SCALE_COUPLING: TensorProblemDefinition(
        ...
        title="Multi‑Scale Coupling of Thermodynamic Tensors",
        scientific_description="Bridge electronic‑scale DFT, atomistic MD, and mesoscale phase‑field models using tensor‑based data transfer.",
        root_cause="Information loss and inconsistency between scales hinders predictive modelling.",
        key_concepts=["density_functional_theory", "molecular_dynamics", "phase_field_model", "gibbs_free_energy", "cluster_expansion"],
        key_relationships=[("density_functional_theory", "TRAINS", "cluster_expansion"),
                           ("cluster_expansion", "MAPS", "phase_field_model")],
        solution_directions=["Tensor‑based surrogate models", "Uncertainty‑aware scale bridging", "Consistent free energy formulations"],
        relevant_descriptors=["composition_space", "temperature", "pressure"],
        relevant_phenomena=["phase_stability"],
        relevant_properties=["gibbs_free_energy", "enthalpy_of_mixing"],
        example_queries=["How to consistently pass Gibbs energy from DFT to phase‑field models?",
                         "Tensor representations for multi‑scale thermodynamics."],
        visualization_focus=["scale_bridge_diagram", "error_propagation"]
    ),
    TensorThermodynamicsProblem.GENERAL: TensorProblemDefinition(
        ...
        scientific_description="General question about thermodynamic tensors, decomposition, or CALPHAD.",
        root_cause="N/A",
        key_concepts=["thermodynamic_data_tensor", "gibbs_tensor"],
        key_relationships=[],
        solution_directions=[],
        relevant_descriptors=[],
        relevant_phenomena=[],
        relevant_properties=[],
        example_queries=["What are thermodynamic data tensors?"],
    ...
    TensorThermodynamicsProblem.MULTI_PROBLEM: TensorProblemDefinition(
        ...
        title="Multi‑Problem Tensor Inquiry",
...
class FallbackAnalyzer(LLMQueryAnalyzer):
    PROBLEM_KEYWORDS = {
        ...
        TensorThermodynamicsProblem.GIBBS_ENERGY_PREDICTION: {"gibbs", "free energy", "surrogate", "pinn", "gpr"},
        ...
        TensorThermodynamicsProblem.CALPHAD_OPTIMIZATION: {"calphad", "optimize", "database", "parameters"},
...
class OpenAIQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        system_prompt = """You are an expert in thermodynamic data tensors and CALPHAD. Analyze the user's query and return ONLY valid JSON with:
        1. "primary_problem": One of: tensor_completion, gibbs_energy_prediction, phase_diagram_prediction, uncertainty_analysis, calphad_optimization, multi_scale_coupling, general, multi_problem
...
class LocalLLMQueryAnalyzer(LLMQueryAnalyzer):
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        if not self.is_available():
            return FallbackAnalyzer().analyze_query(query, ontology)
        prompt = f"[INST] You are an expert in thermodynamic tensors. Analyze: '{query}'. Return ONLY valid JSON with: primary_problem, explicitly_mentioned (snake_case list), inferred_concepts (list), query_type, highlight_paths (list of [src, tgt]), reasoning_chain (list). [/INST]"
        try:
...
class GraphRAGAnswerGenerator:
    ...
    def generate_ground_response(self, query: str, analysis: QueryAnalysisResult, subgraph: nx.Graph,
        ...
        nl = chr(10)
        prompt = "You are an expert in thermodynamic data tensors and CALPHAD. Answer the user's query based *strictly* on the provided graph context and evidence snippets." + nl
        ...
        prompt += "2. Explicitly mention how the key concepts interact (e.g., causal chains like 'tensor completion improves Gibbs energy prediction')." + nl
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    query = st.sidebar.text_area("Your question about thermodynamic tensors:", value=selected_example, height=100, key="llm_query_input", placeholder="e.g., How does tensor completion improve Gibbs energy prediction?")
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    st.markdown("Ask a specific scientific question about thermodynamic tensors, Gibbs energy, or CALPHAD. The system will dynamically expand the ontology, extract a relevant subgraph, and generate a grounded answer using retrieved literature snippets.")
    ...
    with col1:
        query = st.text_input("Enter your research question:", placeholder="e.g., How does tensor completion help predict Gibbs energy?")
    with col2:
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.subheader("🔬 Thermodynamic Tensor Focus Areas")
        st.markdown(r"- **Tensor Methods:** CP, Tucker, completion, ALS, low-rank approximation")
        st.markdown(r"- **Thermodynamic Properties:** Gibbs free energy, excess Gibbs, enthalpy, entropy, chemical potential")
        st.markdown(r"- **CALPHAD:** Redlich‑Kister polynomials, sublattice model, interaction parameters, phase diagrams")
        st.markdown(r"- **Phase‑Field Models:** Allen‑Cahn, Cahn‑Hilliard, KKS, grand potential")
        st.markdown(r"- **AI/Data‑Driven:** PINNs, tensor neural networks, Gaussian processes, active learning, Bayesian optimisation")
        st.markdown(r"- **Multi‑Scale:** DFT, MD, cluster expansion, SQS, scale bridging")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox(
            "Use ontology-based resolution", value=True,
            help="Maps synonyms like 'G', 'free energy' to canonical concepts",
        )
        ...
        st.session_state['use_relationship_extraction'] = st.checkbox(
            ...
            help="Identifies causal links between tensor parameters and thermodynamic properties",
        )
        ...
        if (
            ...
            all_cats = list(set(
                categorize_tensor_concept(c) for c in st.session_state['analysis_data']['valid_concepts']
            ))
...
# ============================================================================
def main() -> None:
    st.title("📊 Thermodynamic Data Tensor Concept Graph v6.1")
    st.caption(
        "Multi-level reasoning concept graph for high-dimensional thermodynamic data tensors | "
        "Focus: Gibbs free energy as a function of composition, temperature, and pressure | "
