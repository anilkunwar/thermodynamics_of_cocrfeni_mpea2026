...
v4.0 MEMORY-OPTIMIZED FOR LARGE DATASETS:
- Chunked parallel document processing (prevents OOM on ≤1GB RAM)
- Disabled concept context storage (optional, saves millions of string objects)
- Lazy all_texts generation (no full text list kept in RAM)
- Heavy analytics skipped for large corpora (>1000 docs)
- PyTorch threads limited to 1 to reduce internal parallelism overhead
- Aggressive garbage collection after each chunk and heavy step

v3.0 improvements retained:
- Removed unused transformers import
- Batched embedding with torch.no_grad() + convert_to_numpy
- Single-pass regex for context extraction
- Parallel processing with ThreadPoolExecutor
- Ontology-aware resolution and reasoning
...
def timed(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        PerformanceMonitor._call_counts[func_name] = PerformanceMonitor._call_counts.get(func_name, 1) + 1
        return result
    return wrapper
...
class DomainOntology:
    ...
    def resolve_concept(self, text: str) -> Optional[str]:
        ...
        variants = [
            ...
            re.sub(r'\bs\b', '', text_lower),  # remove standalone 's'
            re.sub(r'\bes\b', '', text_lower),  # remove standalone 'es'
        ]
...
class EnhancedConceptExtractor:
    ...
    def __init__(self, ontology: DomainOntology, resolver: AdvancedConceptResolver,
        ...
        self.store_context = store_context  # v4.0: optional context storage
        self.concept_frequencies: Dict[str, int] = defaultdict(int)
        self.concept_contexts: Dict[str, List[str]] = defaultdict(list) if store_context else None
    ...
    def extract_from_text(self, text: str, doc_id: int = 0) -> List[str]:
        """Extract concepts using Batch Resolution (50x faster than individual calls)."""
        concepts = set()
        raw_concepts = set()
        ...
        for concept in concepts:
            ...
            # v4.0: Optional context storage – disabled by default to save RAM
    ...
    def _extract_from_context_windows(self, text: str, window_size: int = 100) -> Set[str]:
        """Extract context phrases using the optimized single-regex pattern."""
...
def load_embedding_model():
    ...
    # Limit PyTorch threads to 1 to reduce internal parallelism overhead (v4.0)
...
# ==========================================
# CONCEPT FILTERING & NORMALIZATION
# ==========================================
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    # HEA-specific noun phrase extraction
    ...
    for keyword in ALL_DOMAIN_KEYWORDS:
        for match in re.finditer(r'\b' + re.escape(keyword) + r'\b', text_lower):
            ...
            for phrase in context_phrases:
                ...
                if is_valid_hea_laser_concept(concept):
                    concepts.add(concept)
    # Parameter-value extraction for laser/thermophysical quantities
...
def extract_concepts_from_abstracts(df: pd.DataFrame, text_columns: List[str]) -> Tuple[List[List[str]], List[Dict]]:
    ...
    for idx, row in df.iterrows():
        ...
        # Extract thermophysical quantities
...
def render_sidebar():
    with st.sidebar:
        st.header("⚙️ Configuration v4.0 (Memory Optimized)")
        ...
        # v4.0: Memory optimization controls
        st.subheader("💾 Memory Optimizations")
        st.session_state['chunk_size'] = st.slider("Documents per chunk", 10, 500, 100, step=10,
                                                   help="Smaller chunks reduce memory usage for large datasets")
        st.session_state['store_context'] = st.checkbox("Store concept contexts (snippets)", value=False,
                                                         help="Disable to save memory for large corpora")
        st.session_state['skip_heavy_analytics'] = st.checkbox("Skip heavy analytics (for large data)", value=True,
                                                               help="Skips Tf-Idf, drift, etc. to prevent OOM")
...
# ==========================================
# MAIN APPLICATION v4.0
# ==========================================

def main():
    st.title("🔬 HEA-Laser-ConceptGraph: Advanced NLP-Enhanced Explorer v4.0")
    st.caption("Multi-level reasoning concept graph for CoCrFeNi laser AM | Memory-Optimized for Large Corpora | Parallel Chunked Processing | Interactive Visualization | Ontology-aware resolution")
    ...
    # --- RUN ANALYSIS WITH ENHANCED REASONING & CHUNKED PARALLEL PROCESSING ---
    if st.button("🚀 Build Concept Graph with Reasoning", type="primary", width="stretch"):
        ...
        try:
            with status:
                ...
                store_context = st.session_state.get('store_context', False)  # v4.0: optional context storage

                if use_ontology:
                    st.write("Initializing ontology-based concept resolver...")
                    resolver = AdvancedConceptResolver(ontology, embed_model)
                    extractor = EnhancedConceptExtractor(ontology, resolver, store_context=store_context)
                ...
                st.write("Extracting concepts from abstracts using chunked parallel processing...")
                all_concepts = [None] * len(df_filtered)
                all_metrics = [None] * len(df_filtered)

                if use_ontology and extractor is not None:
                    # v4.0: Chunked processing
                    chunk_size = st.session_state.get('chunk_size', 100)
                    n_chunks = (len(df_filtered) + chunk_size - 1) // chunk_size
                    max_workers = min(4, os.cpu_count() or 4)  # limit concurrency to save memory

                    for chunk_idx in range(n_chunks):
                        start_idx = chunk_idx * chunk_size
                        end_idx = min(len(df_filtered), (chunk_idx+1)*chunk_size)
                        chunk_df = df_filtered.iloc[start_idx:end_idx]
                        status.write(f"Processing chunk {chunk_idx+1}/{n_chunks} (rows {start_idx+1}-{end_idx})")
                        ...
                        progress_bar.progress(0.20 + (end_idx / len(df_filtered)) * 0.15)
                        status.write(f"Extracted {end_idx}/{len(df_filtered)} documents")
                    ...
                    # Use legacy extraction (sequential, no chunking)
                ...
                # v4.0: Conditionally run heavy analytics based on dataset size and user preference
                skip_heavy = st.session_state.get('skip_heavy_analytics', True)
                if not skip_heavy and len(valid_concepts) <= 1000 and num_abstracts <= 2000:
                    st.write("Computing distillation metrics...")
                    # We need all_texts – we can build it on the fly without storing all
                    # Reconstruct texts for distillation only
                    all_texts_for_distill = []
                    for idx, row in df_filtered.iterrows():
                        text = " ".join([str(row[col]) for col in selected_text_cols if col in row and pd.notna(row[col])])
                        all_texts_for_distill.append(text)
                    distill_df = compute_concept_distillation(valid_concepts, concept_abstract_map, all_texts_for_distill)
                    # Free memory
                    del all_texts_for_distill
                    gc.collect()
                else:
                    # Create a minimal distillation DF without heavy computation
                    distill_df = pd.DataFrame({
                        'concept': valid_concepts,
                        'frequency': [len(concept_abstract_map.get(c, [])) for c in valid_concepts],
                        'tfidf_weight': [1.0]*len(valid_concepts),
                        'semantic_density': [1.0]*len(valid_concepts),
                        'coherence_score': [0.5]*len(valid_concepts),
                        'distillation_efficiency': [np.log1p(len(concept_abstract_map.get(c, []))) for c in valid_concepts]
                    }).sort_values('distillation_efficiency', ascending=False)
                    st.info("Heavy analytics skipped to preserve memory.")

                # Advanced analytics – only if not skipped
                if not skip_heavy and len(valid_concepts) <= 500 and num_abstracts <= 1500:
                    st.write("Running advanced analytics (burst, drift, genealogy, bridge, motifs)...")
                ...
                # Store analysis data (excluding large all_texts)
                analysis_data = {
