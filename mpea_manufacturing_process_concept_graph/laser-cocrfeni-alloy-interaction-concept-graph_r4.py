...
- Hierarchical concept taxonomy (hypernym/hyponym relationships)
- Cross-reference reasoning (linking process parameters to microstructure responses)
- Entity disambiguation (distinguishing "phase" in thermodynamics vs. metallurgy)
...
class AdvancedConceptResolver:
    ...
    def _substring_match(self, text: str) -> Optional[str]:
        ...
        for canonical, node in self.ontology.concepts.items():
            ...
            for form in all_forms:
                # Check if text contains form or form contains text
                if form in text or text in form:
                    # Ensure it's a meaningful match (not just short substrings)
    ...
    def _embedding_match(self, text: str, context: str = "") -> Optional[str]:
        ...
        try:
            # Combine text with context for richer representation
            ...
            # Compute similarities with all ontology concepts
            best_match = None
            best_score = 0
    ...
    def _context_disambiguation(self, text: str, context: str) -> Optional[str]:
        ...
        # Check for domain-specific context clues
        ...
        if 'phase' in text:
            if is_thermo and not is_microstructure:
                return "gibbs_energy"  # thermodynamic phase
            elif is_microstructure and not is_thermo:
                return "fcc_phase"  # structural phase
            elif is_fluid:
                return "liquid_phase"  # fluid phase

        # "Interface" disambiguation
        if 'interface' in text:
            if is_thermo:
                return "kks_model"  # phase-field interface
            elif is_microstructure:
                return "grain_boundary"  # grain interface
...
class EnhancedConceptExtractor:
    ...
    def extract_from_text(self, text: str, doc_id: int = 0) -> List[str]:
        ...
        # Level 3: Noun phrase extraction for domain terms
    ...
    def _extract_noun_phrases(self, text: str) -> Set[str]:
        ...
        # Pattern: (adjective/noun)+ (noun) combinations common in materials science
        ...
        # Also capture compound terms with prepositions
...
class ReasoningEnhancedGraphBuilder:
        ...
        try:
            ...
            for i, c1 in enumerate(valid_concepts):
                for j, c2 in enumerate(valid_concepts[i+1:], start=i+1):
                    ...
                    sim = sim_matrix[i][j]
                    if sim > sim_thresh:
                        # Check if nodes have low degree (bridge concepts)
    ...
    def _infer_cross_domain_bridges(self, nx_graph: nx.Graph, valid_concepts: List[str]):
        ...
        # Find process nodes and property nodes
        ...
        # Infer process -> property connections through shared intermediates
        for proc in process_nodes:
            for prop in property_nodes:
                if not nx_graph.has_edge(proc, prop):
                    # Check if there's a path through the ontology
                    paths = self.ontology.infer_path(proc, prop, max_depth=2)
                    if paths:
                        # Add inferred bridge edge
                        avg_confidence = 0.6  # default confidence for inferred bridges
    ...
    def _add_cause_effect_edges(self, nx_graph: nx.Graph):
        ...
        # This would be populated during document processing
        # For now, we rely on the ontology-defined causal chains
        pass
    ...
    def _add_hierarchical_edges(self, nx_graph: nx.Graph, valid_concepts: List[str]):
        ...
        for concept in valid_concepts:
            ...
            # Add hypernym edges (is-a relationships)
...
def get_hea_laser_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    concept_lower = concept.lower()
    # Use HEA_CATEGORY_MAPPING to determine category, then assign color
    category = 'general'
    for pattern, cat in HEA_CATEGORY_MAPPING.items():
        if re.search(pattern, concept_lower):
            ...
            break
    color_map = {
        'core_material': '#D32F2F',           # Red
        'manufacturing_process': '#00BCD4',     # Cyan
        'thermodynamics_tensor': '#FF9800',     # Orange
        'phase_field_modeling': '#9C27B0',      # Purple
        'fluid_dynamics_melt_pool': '#2196F3',  # Blue
        'ai_surrogate_model': '#8E24AA',         # Deep Purple
        'microstructural_feature': '#4CAF50',   # Green
        'computational_method': '#795548',       # Brown
        'general': '#9E9E9E'                    # Grey
    }
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=11, width=800, height=600, theme=None, branchvalues="total"):
    ...
    fig.update_layout(
        title="<b>CoCrFeNi HEA Laser AM Process-Material Response Domain Hierarchy</b><br><i>Size = concept frequency</i>",
...
def export_graph(nx_graph, concept_abstract_map, format_type: str):
    if format_type == "GraphML":
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "hea_edges.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "hea_nodes.csv"
...
# ==========================================
def main():
    st.title("HEA-Laser-ConceptGraph: Advanced NLP-Enhanced Explorer")
    st.caption("Multi-level reasoning concept graph for CoCrFeNi laser AM | Ontology-aware resolution | Cause-effect extraction")
    ...
    # --- RUN ANALYSIS WITH ENHANCED REASONING ---
        ...
        try:
            with status:
                ...
                if use_ontology and extractor is not None:
                    ...
                    for idx, row in df_filtered.iterrows():
                        text = " ".join([str(row[col]) for col in selected_text_cols 
                                        if col in row and pd.notna(row[col])])
