...
# -*- coding: utf-8 -*-
"""
HEA-Laser-ConceptGraph v6.0 (Faithful AgNPs Architecture Port + Batch Mode)
==============================================================================
...
def compute_concept_distillation(
    ...
    all_texts: List[str],
    max_docs_per_concept: int = 0,
) -> pd.DataFrame:
    distill_data: List[Dict[str, Any]] = []
    doc_corpus: List[str] = []
    for c in valid_concepts:
        doc_indices = concept_abstract_map.get(c, [])
        # v6.0: optional cap keeps TF-IDF corpus bounded in batch mode (<=1 GB)
...
def render_batch_processing_controls() -> None:
    ...
    if bs:
        ...
        st.caption(
            f"Batch {done}/{total} • "
            f"{len(bs.get('all_texts', []))} docs processed"
        )
...
def run_batch_analysis(
    ...
    def _finalize() -> None:
        ...
        distill_df = compute_concept_distillation(
            ...
            max_docs_per_concept=60,  # bounded TF-IDF corpus (≤1 GB)
        )
        burst_df = None
        drift_df = None
        genealogy_df = None
...
def render_sidebar() -> None:
    with st.sidebar:
        st.header("⚙️ Configuration v6.0")
        st.subheader("🎨 Theme")
