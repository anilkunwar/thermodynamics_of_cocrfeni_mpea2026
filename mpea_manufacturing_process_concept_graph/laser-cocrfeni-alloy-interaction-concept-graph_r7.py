...
# -*- coding: utf-8 -*-
"""
HEA-Laser-ConceptGraph: Advanced NLP-Enhanced Concept Graph Builder
====================================================================
v4.0 OOM-HARDENED EDITION
====================================================================
Multi-level reasoning concept graph for CoCrFeNi laser AM process-material response.

v4.0 OOM FIXES (vs v3.0):
- _embedding_match rewritten to use precomputed matrix (no per-concept loop)
- Query embedding cache REMOVED (only ontology embeddings cached)
- convert_to_numpy=True on EVERY embed_model.encode() call
- torch.no_grad() wrapped around ALL inference
- Parallel workers reduced to 4 (shared cache by reference)
- PyVis HTML streamed to temp file (no 100MB string in RAM)
- concept_contexts capped at 5 entries per concept
- Sunburst get_depth() memoized
- Periodic gc.collect() + torch.cuda.empty_cache() at checkpoints
- Batch size reduced 64 -> 32 for large corpora
- top_n_nodes auto-capped at 400 for PyVis

v3.0 fixes (preserved):
- Removed unused transformers import
- Moved time/functools imports to top
- Added periodic gc.collect() + torch.cuda.empty_cache()
- Replaced deprecated use_container_width -> width="stretch"
- Added convert_to_numpy=True in encode() calls

DEPLOYMENT:
pip install streamlit torch sentence-transformers networkx scikit-learn
pip install pyvis plotly pandas numpy kaleido matplotlib scipy seaborn bibtexparser
Run: streamlit run hea_laser_concept_graph_enhanced.py
...
class AdvancedConceptResolver:
    """
    v4.0 OOM-HARDENED:
    - _embedding_match uses precomputed matrix (NO per-concept loop)
    - Query embedding cache REMOVED (only ontology embeddings cached)
    - convert_to_numpy=True on every encode() call
    ...
    def __init__(self, ontology: DomainOntology, embed_model):
        ...
        # v4.0: REMOVED self.embedding_cache for queries (was the OOM source)
        # Only ontology concept embeddings are cached in the matrix
    ...
    def _precompute_ontology_embeddings(self):
        ...
        for canonical, node in self.ontology.concepts.items():
            ...
            with torch.no_grad():
                emb = self.embed_model.encode(texts, show_progress_bar=False,
    ...
    def resolve(self, text: str, context: str = "", use_embedding: bool = True) -> Optional[str]:
        ...
        if use_embedding and self.embed_model is not None and self.ontology_embedding_matrix.size > 0:
            # v4.0: Use matrix-based match (NO per-concept loop)
    ...
    # ============================================================
    # v4.0 FIX: _embedding_match uses MATRIX OPERATION (no loop)
    # ============================================================
    def _embedding_match(self, text: str, context: str = "") -> Optional[str]:
        """
        v4.0 OOM FIX: Uses precomputed matrix + single query encode.
        NO per-concept loop. NO query cache.
        """
        ...
        try:
            ...
            # Matrix cosine similarity — single vectorized op
...
class EnhancedConceptExtractor:
    """
    v4.0: concept_contexts capped at 5 entries per concept.
    """
    ...
    def _build_extraction_patterns(self):
        self.alloy_patterns = [
            r'\bco(?:-?|\s+)cr(?:-?|\s+)fe(?:-?|\s+)ni\b', r'\bcocrfeni\b',
            r'\bcobalt\s+chromium\s+iron\s+nickel\b', r'\bhigh[-\s]?entropy[-\s]?alloy[s]?\b',
            r'\bhea[s]?\b', r'\bmulti[-\s]?principal[-\s]?element[-\s]?alloy[s]?\b',
            r'\bmpea[s]?\b', r'\bquaternary[-\s]?alloy[s]?\b',
            r'\bcomplex[-\s]?concentrated[-\s]?alloy[s]?\b', r'\bcca[s]?\b',
        ]
        self.process_patterns = [
            r'\blaser[-\s]?powder[-\s]?bed[-\s]?fusion\b', r'\blpbf\b',
            r'\bselective[-\s]?laser[-\s]?melting\b', r'\bslm\b',
            r'\blaser[-\s]?additive[-\s]?manufacturing\b', r'\blam\b',
            r'\badditive[-\s]?manufacturing\b', r'\bdirected[-\s]?energy[-\s]?deposition\b',
            r'\bded\b', r'\blaser[-\s]?(?:melting|solidification|processing|scanning|treatment)\b',
            r'\brapid[-\s]?(?:heating|cooling|solidification)\b', r'\bthermal[-\s]?cycling\b',
        ]
        self.thermo_patterns = [
            r'\bthermodynamic[-\s]?data[-\s]?tensor[s]?\b', r'\btdt\b',
            r'\bcanonical[-\s]?polyadic[-\s]?decomposition\b', r'\bcpd\b', r'\bparafac\b',
            r'\bgibbs[-\s]?(?:free[-\s]?)?energy\b', r'\bcalphad\b',
            r'\bphase[-\s]?conditioned[-\s]?composition[-\s]?tensor\b', r'\bctf\b',
            r'\bchemical[-\s]?driving[-\s]?(?:force|pressure)\b',
            r'\bcapillary[-\s]?(?:resistance|pressure|force)\b',
            r'\binterfacial[-\s]?energy\b', r'\bsurface[-\s]?tension\b',
            r'\bmolar[-\s]?volume\b', r'\bchemical[-\s]?potential\b',
            r'\bexcess[-\s]?mixing\b', r'\binteraction[-\s]?parameter[s]?\b',
        ]
        self.pf_patterns = [
            r'\bphase[-\s]?field[-\s]?(?:model|simulation|method|modeling)\b', r'\bpfm\b',
            r'\ballen[-\s]?cahn\b', r'\bkks[-\s]?(?:phase[-\s]?equilibrium)?\b',
            r'\bkim[-\s]?kim[-\s]?suzuki\b', r'\bdiffuse[-\s]?interface\b',
            r'\border[-\s]?parameter\b', r'\bmulti[-\s]?component[-\s]?diffusion\b',
            r'\binterface[-\s]?mobility\b', r'\bphase[-\s]?mobility\b',
            r'\bfree[-\s]?energy[-\s]?functional\b',
            r'\bgradient[-\s]?energy[-\s]?coefficient\b',
            r'\blandau[-\s]?polynomial\b', r'\bbarrier[-\s]?function\b',
        ]
        self.fluid_patterns = [
            r'\bmarangoni[-\s]?(?:convection|flow|effect|force)\b',
            r'\bthermocapillary[-\s]?(?:convection|flow)\b', r'\bnavier[-\s]?stokes\b',
            r'\bmelt[-\s]?pool[-\s]?(?:morphology|dynamics|flow|depth|width|shape|geometry)\b',
            r'\bsurface[-\s]?tension[-\s]?gradient\b',
            r'\bboussinesq[-\s]?(?:approximation)?\b', r'\bincompressible[-\s]?flow\b',
        ...
        self.ai_patterns = [
            ...
            r'\battention[-\s]?(?:mechanism|regularized)?\b', r'\bcross[-\s]?attention\b',
            r'\bgaussian[-\s]?locality\b', r'\bdigital[-\s]?twin\b',
            r'\bmachine[-\s]?learning\b', r'\bdeep[-\s]?learning\b',
            r'\bneural[-\s]?network\b', r'\bphysics[-\s]?(?:informed|guided)\b',
            r'\bdata[-\s]?driven\b', r'\bmulti[-\s]?head[-\s]?attention\b',
        ]
        self.micro_patterns = [
            r'\bmicrostructural[-\s]?evolution\b', r'\belemental[-\s]?partitioning\b',
            r'\bsolidification[-\s]?kinetics\b', r'\bequiaxed[-\s]?grain[s]?\b',
            r'\bcolumnar[-\s]?grain[s]?\b', r'\bgrain[-\s]?boundary\b',
            r'\bthermal[-\s]?wake\b', r'\bcooling[-\s]?rate\b',
            r'\bphase[-\s]?transformation\b', r'\bnucleation\b',
            r'\binterface[-\s]?motion\b', r'\bgrain[-\s]?size\b', r'\bsegregation\b',
            r'\bnon[-\s]?equilibrium[-\s]?microstructure[s]?\b',
            r'\bhot[-\s]?(?:tearing|cracking)\b', r'\bporosity\b',
            r'\bdendrite\b', r'\bdendritic[-\s]?growth\b',
        ]
        self.comp_patterns = [
            r'\bfinite[-\s]?element[-\s]?(?:analysis|method)?\b', r'\bfea\b', r'\bfem\b',
            r'\bmoose[-\s]?(?:framework)?\b', r'\balternating[-\s]?least[-\s]?squares\b',
            r'\bals\b', r'\btensor[-\s]?factorization\b',
            r'\bmulti[-\s]?linear[-\s]?interpolation\b', r'\bhessian[-\s]?matrix\b',
            r'\bspectral[-\s]?decomposition\b', r'\brank[-\s]?1[-\s]?outer[-\s]?product[s]?\b',
            r'\brmse\b', r'\broot[-\s]?mean[-\s]?square[-\s]?error\b',
            r'\bleave[-\s]?one[-\s]?out[-\s]?cross[-\s]?validation\b',
            r'\bdice[-\s]?coefficient\b', r'\bintersection[-\s]?over[-\s]?union\b',
            r'\biou\b', r'\bdiscretization\b', r'\bmesh\b',
        ]
        self.param_patterns = [
    ...
    def extract_from_text(self, text: str, doc_id: int = 0) -> List[str]:
        ...
        for concept in concepts:
            ...
            # v4.0: Cap context storage at 5 entries per concept
            ctx_list = self.concept_contexts[concept]
            if len(ctx_list) < 5:
                ctx_list.append(text[:200])
            ...
            for match in matches:
                if len(match) >= 2:
                    ...
                    if source_canon and target_canon and source_canon != target_canon:
                        rel = Relationship(
                            source=source_canon, target=target_canon,
                            rel_type=RelationshipType.CAUSES, confidence=0.7,
                            evidence=text[:150]
                        )
...
class ReasoningEnhancedGraphBuilder:
    ...
    def build_graph(self, all_concepts: List[List[str]], valid_concepts: List[str],
        ...
        for (u, v), count in cooccurrence_map.items():
            nx_graph.add_edge(u, v, weight=count, cooccurrence=count, semantic=0,
            ...
            if rel.source in valid_concepts and rel.target in valid_concepts:
                if not nx_graph.has_edge(rel.source, rel.target):
                    nx_graph.add_edge(rel.source, rel.target,
                                      weight=rel.confidence * 2, cooccurrence=0,
                                      semantic=rel.confidence, edge_type=rel.rel_type.value,
                                      inferred=True, confidence=rel.confidence)
        ...
        for proc in process_nodes:
            for prop in property_nodes:
                if not nx_graph.has_edge(proc, prop):
                    ...
                    if paths:
                        avg_confidence = 0.6
                        nx_graph.add_edge(proc, prop, weight=avg_confidence, cooccurrence=0,
                                          semantic=avg_confidence, edge_type='bridge',
                                          inferred=True, path=" -> ".join(paths[0]))
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    noun_pattern = r'\b(?:[A-Z][a-z]+(?:\d+(?:\.\d+)?)?[\s\-]?){1,3}(?:alloy|composition|tensor|parameter|gradient|energy|force|pressure|diffusion|interface|mobility|microstructure|grain|phase|melt\s+pool|surrogate|model|simulation|method|analysis|optimization)\b'
    ...
    for keyword in ALL_DOMAIN_KEYWORDS:
        for match in re.finditer(r'\b' + re.escape(keyword) + r'\b', text_lower):
            ...
            for phrase in context_phrases:
                ...
                if is_valid_hea_laser_concept(concept):
                    concepts.add(concept)
    param_pattern = r'\b([a-z\s]+(?:temperature|velocity|power|density|gradient|energy|pressure|force|viscosity|conductivity|capacity|tension))\s+(?:of|is|=|:)?\s*(\d+(?:\.\d+)?\s*(?:k|mpa|gpa|w|m/s|mm/s|j|pa|nm|µm|um|k/w|w/mk|j/kgk|pa·s|n/m))\b'
    ...
    for concept in concepts:
        ...
        for pattern, category in HEA_CATEGORY_MAPPING.items():
            if re.search(pattern, concept, re.I):
                ...
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'\bcocrfeni', r'\bhea', r'\bmpea', r'\bhigh[\s-]entropy']):
                concept_to_abstract[concept] = 'core_material'
            elif any(re.search(p, concept, re.I) for p in [r'\blaser', r'\blpbf', r'\blam', r'\bmelt\s+pool', r'\brapid\s+(?:heating|cooling)']):
            ...
            elif any(re.search(p, concept, re.I) for p in [r'\bphase[\s-]field', r'\ballen[\s-]cahn', r'\bkks', r'\bdiffuse\s+interface']):
                concept_to_abstract[concept] = 'phase_field_modeling'
            elif any(re.search(p, concept, re.I) for p in [r'\bmarangoni', r'\bnavier[\s-]stokes', r'\bfluid', r'\bsurface\s+tension']):
                concept_to_abstract[concept] = 'fluid_dynamics_melt_pool'
            elif any(re.search(p, concept, re.I) for p in [r'\bai\s+surrogate', r'\btransformer', r'\battention', r'\bdigital\s+twin', r'\bmachine\s+learning', r'\bdeep\s+learning']):
                concept_to_abstract[concept] = 'ai_surrogate_model'
            elif any(re.search(p, concept, re.I) for p in [r'\bmicrostructural', r'\bsolidification', r'\bgrain', r'\bsegregation', r'\bpartitioning']):
                concept_to_abstract[concept] = 'microstructural_feature'
            elif any(re.search(p, concept, re.I) for p in [r'\bfea', r'\bmoose', r'\bfinite\s+element', r'\btensor\s+factorization']):
    ...
    for concept in valid_concepts:
        ...
        if burst_scores:
            ...
            if max_burst >= burst_threshold:
                burst_data.append({
                    "concept": concept, "burst_score": round(max_burst, 2),
...
def detect_semantic_drift(df_filtered: pd.DataFrame, valid_concepts: List[str],
    ...
    for concept in valid_concepts:
        ...
        try:
            ...
            drift_data.append({
                "concept": concept, "semantic_drift": round(float(drift), 4),
...
def export_publication_figure(nx_graph, valid_concepts, concept_abstract_map,
                              ...
                              filename="hea_graph_pub.png") -> bytes:
    try:
...
def get_hea_laser_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    for pattern, cat in HEA_CATEGORY_MAPPING.items():
        if re.search(pattern, concept_lower):
            ...
            break
    color_map = {
        'core_material': '#D32F2F', 'manufacturing_process': '#00BCD4',
        'thermodynamics_tensor': '#FF9800', 'phase_field_modeling': '#9C27B0',
        'fluid_dynamics_melt_pool': '#2196F3', 'ai_surrogate_model': '#8E24AA',
        'microstructural_feature': '#4CAF50', 'computational_method': '#795548',
        'general': '#9E9E9E'
    }
    return color_map.get(category, '#9E9E9E')

# ==========================================
# v4.0 OOM FIX: PyVis HTML streamed to temp file
# ==========================================
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    """
    v4.0 OOM FIX:
    - Auto-cap top_n_nodes at 400 for huge graphs
    - HTML written to temp file, then streamed (no giant string in RAM)
    """
    # v4.0: Auto-cap for huge graphs
    if top_n_nodes == 0 and len(nx_graph.nodes()) > 400:
        top_n_nodes = 400
        st.info(f"🧹 Auto-capping graph at 400 nodes to prevent HTML OOM. Full graph: {len(nx_graph.nodes())} nodes.")
...
""")
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        if use_abbreviated_labels and len(original_label) > max_label_length:
            ...
            node_shape = 'dot'
            font_dict = {'color': theme['font'], 'size': node_label_size, 'face': node_font_face,
                         'strokeWidth': 0, 'vadjust': -6}
...
<span style='color:{theme['tooltip_text']};opacity:0.7;'>Frequency:</span> {freq}"""
    ...
    for u, v in nx_graph.edges():
        ...
        edge_kwargs = dict(
            ...
            color={'color': color, 'highlight': theme['highlight_bg'],
        ...
        if edge_label_mode == "all" or (edge_label_mode == "threshold" and w >= weight_threshold):
            ...
            edge_kwargs['font'] = {
                'color': actual_edge_label_color, 'size': int(edge_label_size),
    ...
    # ============================================================
    # v4.0 OOM FIX: Write HTML to temp file, then stream
    # ============================================================
    try:
        # Generate HTML directly to file
        ...
        # Read back and inject custom CSS/JS (in chunks to avoid giant string)
...
"""
        ...
        # Offer download
        try:
            st.download_button("Download Interactive Graph (HTML)", data=html_content.encode('utf-8'),
...
def render_graph_plotly_2d(nx_graph, concept_abstract_map, cmap_name="viridis",
    ...
    if show_edge_weights:
        for u, v in nx_graph.edges():
            ...
            fig_data.append(go.Scatter(x=[mid_x], y=[mid_y], mode='text',
                                       text=[f"{w:.1f}"],
...
def render_graph_plotly_3d(nx_graph, concept_abstract_map, cmap_name="viridis", top_n_nodes=0,
    ...
    if show_edge_weights:
        for u, v in nx_graph.edges():
            ...
            fig_data.append(go.Scatter3d(x=[mid_x], y=[mid_y], z=[mid_z], mode='text',
                                         text=[f"{w:.1f}"],
...
# ==========================================
# v4.0 OOM FIX: Sunburst with memoized depth
# ==========================================
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=11,
    ...
    fig = go.Figure(go.Sunburst(
        ...
        marker=dict(colors=plot_colors, line=dict(width=0.5, color="rgba(255,255,255,0.25)")),
        textinfo="label+percent entry+value", insidetextorientation="radial",
    ...
    fig.update_layout(
        ...
        width=width, height=height, margin=dict(t=80, b=20, l=20, r=20)
    )
    st.plotly_chart(fig, width="stretch")
...
def export_graph(nx_graph, concept_abstract_map, format_type: str, include_metadata: bool = True):
    if format_type == "GraphML":
        ...
        if include_metadata:
            data['metadata'] = {
                'created': datetime.now().isoformat(), 'version': '4.0',
        ...
        edge_data = []
        for u, v, data in nx_graph.edges(data=True):
            row = {"source": u, "target": v, "weight": data.get('weight', 1),
...
def render_concept_growth(df_filtered, valid_concepts, concept_abstract_map, theme=None):
    ...
    for concept in top_concepts:
        ...
        growth_data.append({
            "Concept": concept, "Early Count": early_count,
            "Recent Count": recent_count, "Growth Rate (%)": growth_rate
        })
    ...
    fig = px.bar(growth_df, x="Concept", y="Growth Rate (%)",
                 ...
                 template="plotly_white" if theme == THEME_PRESETS["Bright (Default)"] else "plotly_dark")
    fig.update_layout(paper_bgcolor=theme.get("plotly_paper", "#ffffff"),
                      font_color=theme.get("font", "#000000"), xaxis_tickangle=-45)
    st.plotly_chart(fig, width="stretch")
...
def render_bubble_chart(nx_graph, valid_concepts, concept_abstract_map, distill_df, theme=None):
    ...
    fig = px.scatter(bubble_df, x="Degree", y="Frequency", size="Distillation Efficiency",
                     ...
                     title="Concept Importance Bubble Chart", size_max=50,
                     template="plotly_white" if theme == THEME_PRESETS["Bright (Default)"] else "plotly_dark")
    fig.update_layout(paper_bgcolor=theme.get("plotly_paper", "#ffffff"),
                      font_color=theme.get("font", "#000000"))
    st.plotly_chart(fig, width="stretch")
...
def render_sidebar():
    with st.sidebar:
        ...
        with st.expander("Merge Nodes"):
            if st.session_state.get('analysis_data') and st.session_state['analysis_data'].get('valid_concepts'):
                nodes_to_merge = st.multiselect("Select nodes to merge:",
                                                options=st.session_state['analysis_data']['valid_concepts'],
...
def render_reasoning_dashboard(nx_graph, valid_concepts, ontology, extractor):
    ...
    if rel_types:
        ...
        fig = px.bar(rel_df, x='Relationship Type', y='Count',
                     title="Edge Type Distribution", color='Relationship Type')
        st.plotly_chart(fig, width="stretch")
    ...
    for proc in process_nodes[:5]:
        for prop in property_nodes[:5]:
            ...
            if paths:
                chains_found.append({
                    "Process": proc, "Property": prop,
                    "Path Length": len(paths[0]), "Path": " → ".join(paths[0])
                })
    if chains_found:
    ...
    synonym_examples = [
        ("high entropy alloy", "hea"), ("co-cr-fe-ni", "cocrfeni"),
    ...
    for original, expected in synonym_examples:
        ...
        syn_data.append({
            "Original": original, "Expected": expected, "Resolved": resolved,
...
# ==========================================
# v4.0 OOM FIX: Shared-cache parallel worker
# ==========================================
def _process_single_row(args):
    """Process a single document row. extractor is shared by reference (threads, not processes)."""
...
def main():
    st.title("🔬 HEA-Laser-ConceptGraph: Advanced NLP-Enhanced Explorer v4.0")
    st.caption("Multi-level reasoning concept graph for CoCrFeNi laser AM | **OOM-HARDENED** | Memory-Optimized | Parallel Processing | Interactive Visualization | Ontology-aware resolution")
        ...
        try:
            with status:
                ...
                if use_ontology and extractor is not None:
                    # v4.0: REDUCED workers to 4 (was 8) — shared cache by reference
                    ...
                    if use_parallel:
                        ...
                        # v4.0: Memory checkpoint after extraction
        ...
        with tabs[tab_idx]:
            ...
            if viz_choice == "PyVis (Interactive)":
                render_graph_pyvis(
                    ...
                    physics_enabled=physics, cmap_name=cmap, top_n_nodes=top_n,
                    theme=theme, physics_preset=physics_preset,
                    show_edge_weights=show_weights, edge_label_mode=edge_label_mode,
                    use_abbreviated_labels=use_abbreviated, max_label_length=max_label_len,
                    node_font_face=node_font, edge_label_size=edge_label_size,
                    edge_label_color=edge_label_color, edge_label_position=edge_label_pos,
