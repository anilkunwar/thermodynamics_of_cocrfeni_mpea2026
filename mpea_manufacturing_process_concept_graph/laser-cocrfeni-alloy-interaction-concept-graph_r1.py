...
# ==========================================
st.set_page_config(
    page_title="HEA-LaserAM-ConceptGraph: CoCrFeNi HEA & Laser AM Explorer",
    page_icon="🔬",
    layout="wide",
...
def generate_analysis_report(nx_graph, valid_concepts, concept_abstract_map, 
    ...
    report.append("---")
    report.append("*Report generated by HEA-LaserAM-ConceptGraph*")
    return "\n".join(report)
...
def get_hea_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    concept_lower = concept.lower()
    if any(c in concept_lower for c in ['cocrfeni', 'co-cr-fe-ni', 'hea', 'mpea', 'high entropy', 'multi-principal', 'cobalt', 'chromium', 'iron', 'nickel']):
        return "#D32F2F"  # Red - Core material
    elif any(c in concept_lower for c in ['laser', 'lpbf', 'lam', 'additive manufacturing', 'melt pool', 'rapid solidification', 'thermal cycling']):
        return "#00BCD4"  # Cyan - Manufacturing
    elif any(c in concept_lower for c in ['tdt', 'cpd', 'calphad', 'gibbs energy', 'ctf', 'thermodynamic', 'chemical driving', 'interfacial energy']):
        return "#FF9800"  # Orange - Thermodynamics
    elif any(c in concept_lower for c in ['phase-field', 'allen-cahn', 'kks', 'diffuse interface', 'order parameter', 'multicomponent diffusion']):
        return "#9C27B0"  # Purple - Phase field
    elif any(c in concept_lower for c in ['marangoni', 'navier-stokes', 'melt pool flow', 'surface tension', 'boussinesq', 'fluidic']):
        return "#3F51B5"  # Indigo - Fluid dynamics
    elif any(c in concept_lower for c in ['ai surrogate', 'transformer', 'attention', 'digital twin', 'machine learning', 'deep learning', 'gaussian locality']):
        return "#8E24AA"  # Deep purple - AI
    elif any(c in concept_lower for c in ['microstructural', 'elemental partitioning', 'solidification', 'equiaxed', 'columnar', 'grain', 'segregation', 'nucleation']):
        return "#4CAF50"  # Green - Microstructure
    elif any(c in concept_lower for c in ['fea', 'moose', 'als', 'tensor factorization', 'rmse', 'cross-validation', 'dice coefficient', 'finite element']):
        return "#795548"  # Brown - Computational
    elif any(c in concept_lower for c in ['thermal conductivity', 'electrical conductivity', 'thermal expansion']):
        return "#FFC107"  # Amber - Transport
    elif any(c in concept_lower for c in ['corrosion', 'oxidation', 'electrochemical']):
        return "#FFEB3B"  # Yellow - Corrosion
    else:
        return "#9E9E9E"  # Gray - General
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    try:
        ...
        st.download_button("Download Interactive Graph (HTML)", data=html_bytes,
                          file_name="hea_laseram_concept_graph.html", mime="text/html")
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown("- Laser Powder Bed Fusion (LPBF) / Laser AM")
        st.markdown("- Thermodynamic Data Tensor (TDT) & CPD")
        st.markdown("- Phase-Field Modeling (PFM, Allen-Cahn, KKS)")
        st.markdown("- Marangoni Convection & Melt Pool Dynamics")
        st.markdown("- AI Surrogate Models & Digital Twins")
        st.markdown("- Microstructural Evolution & Solidification")
        st.markdown("- Computational Methods (FEA/MOOSE, ALS)")
        st.markdown("- Cross-domain: Physics-to-Data Bridge")
        st.markdown("- Cross-domain: Multiphysics Coupling")
...
# ==========================================
def main():
    st.title("HEA-LaserAM-ConceptGraph: CoCrFeNi HEA & Laser AM Explorer")
    st.caption("Large-corpus concept graph builder for CoCrFeNi High-Entropy Alloy & Laser Additive Manufacturing | Optimized for TDT/CPD thermodynamics, phase-field modeling, Marangoni fluid dynamics, and AI surrogate digital twins")
    render_sidebar()
        ...
        with export_tab:
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button("Download Report (Markdown)", data=report.encode('utf-8'),
                                  file_name="hea_laseram_analysis_report.md", mime="text/markdown")
