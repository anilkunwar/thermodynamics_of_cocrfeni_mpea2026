...
# -*- coding: utf-8 -*-
"""
HEA-Laser-ConceptGraph v6.3.0 (Permanent OOM Fix – Disk Offload + Graph Pruning)
===============================================================================
FIXES the remaining OOM issues in v6.2.1:
- Removes `bs["all_metrics"]`, `bs["all_texts"]`, `bs["all_concepts"]`
- Writes texts and metrics to temporary files (disk offload)
- Keeps a BOUNDED `concept_abstract_map` and `concept_texts` (max 30 docs/concept)
- Prunes the NetworkX graph after EVERY batch to keep only top-K nodes
- Caps PyVis rendering to a safe node limit (default 200)

Now the app runs smoothly on Streamlit Cloud free tier with 1 GB RAM.
"""
# ============================================================================
# IMPORTS
# ============================================================================
...
# ============================================================================
st.set_page_config(
    page_title="HEA-Laser-ConceptGraph v6.3.0: Permanent OOM Fix",
    page_icon="🔬",
    layout="wide",
...
class ReasoningEnhancedGraphBuilder:
    ...
    def build_graph(
        ...
        for concepts in all_concepts:
            ...
            for c in valid_in_doc:
                nx_graph.nodes[c]['frequency'] = (
                    nx_graph.nodes[c].get('frequency', 0) + 1
                )
...
def compute_concept_distillation(
    ...
    if texts_is_dict and all_texts:
        ...
        temp_list = [""] * (max_idx + 1)
        for idx, text_val in all_texts.items():
            temp_list[idx] = text_val
        all_texts = temp_list
        texts_is_dict = False
        gc.collect()
...
def render_graph_pyvis(
    ...
    """
    Faithful AgNPs-pattern PyVis renderer with automatic node capping.
    """
    # --- CAP NODES TO AVOID HTML EXPLOSION ---
    ...
    elif top_n_nodes == 0 and len(nx_graph.nodes()) > 500:
        st.warning(f"Graph has {len(nx_graph.nodes())} nodes – limiting to 500 for rendering to avoid memory crash.")
        degrees = dict(nx_graph.degree(weight='weight'))
        top_nodes = sorted(
            degrees.keys(), key=lambda x: degrees[x], reverse=True
        )[:500]
...
"""
    ...
    # Glassmorphism JS with label-mode switching (unchanged)
...
# ============================================================================
# MISSING COMPONENTS FOR BATCH PROCESSING (v6.3.0 Fix)
# ============================================================================
...
def extract_doc_metrics(text: str) -> Dict[str, Any]:
    """Extract numerical metrics from a single document text."""
    metrics: Dict[str, Any] = {}
    ...
    if temp_matches:
        metrics['temperature'] = [float(m) for m in temp_matches]
    energy_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:j|kj|mj)', text, re.I)
    if energy_matches:
        metrics['energy'] = [float(m) for m in energy_matches]
    pressure_matches = re.findall(r'(\d+(?:\.\d+)?)\s*(?:mpa|gpa|pa)', text, re.I)
    if pressure_matches:
        metrics['pressure'] = [float(m) for m in pressure_matches]
    return metrics
...
def merge_graphs(g1: nx.Graph, g2: nx.Graph) -> nx.Graph:
    """Merge two NetworkX graphs, summing edge weights and combining node attributes."""
    merged = g1.copy()
    # Add nodes from g2 with their attributes
    for node, attr in g2.nodes(data=True):
        if node not in merged:
            merged.add_node(node, **attr)
        else:
            # Merge frequency
            merged.nodes[node]['frequency'] = (
                merged.nodes[node].get('frequency', 0)
                + attr.get('frequency', 0)
            )
    # Add edges from g2, summing weights for duplicate edges
    for u, v, attr in g2.edges(data=True):
        if merged.has_edge(u, v):
            for key in ['weight', 'cooccurrence', 'semantic']:
                if key in attr:
                    merged[u][v][key] = merged[u][v].get(key, 0) + attr[key]
            # Keep the more specific edge_type
            if attr.get('edge_type', 'unknown') != 'unknown':
                merged[u][v]['edge_type'] = attr['edge_type']
            if attr.get('inferred', False):
                merged[u][v]['inferred'] = True
        else:
...
def recompute_edge_weights(nx_graph: nx.Graph, config: Dict) -> None:
    """Recompute all edge weights using the configured weighting scheme."""
    cooc_weight = config.get("COOCCURRENCE_WEIGHT", 0.7)
    sem_weight = config.get("SEMANTIC_WEIGHT", 0.2)
    inf_weight = config.get("INFERENCE_WEIGHT", 0.1)
    for u, v, data in nx_graph.edges(data=True):
...
class IncrementalGraphBuilder:
    """Builds concept graphs incrementally from batches of documents.

    This is a memory-efficient variant of ReasoningEnhancedGraphBuilder
    designed for batch processing where graphs are built per-batch and
    then merged.
    """
    ...
    def build_batch_graph(
        ...
        """Build a graph from a single batch of documents."""
        if config is None:
            config = get_adaptive_config(3000)
        nx_graph = nx.Graph()
        ...
        # Add semantic edges if model available
        if embed_model and len(valid_concepts) >= 10:
            self._add_semantic_edges(nx_graph, valid_concepts, embed_model, config)

        # Add inferred edges from ontology
        if st.session_state.get('use_inference', True):
            self._add_inferred_edges(nx_graph, valid_concepts)
...
# ============================================================================
# BATCH PROCESSING MODE v6.3.0 (Permanent OOM Fix)
# ============================================================================
# Helper functions for disk offload
def write_text_to_tempfile(temp_dir: str, doc_id: int, text: str) -> None:
    """Append a document text to a temporary file."""
    with open(os.path.join(temp_dir, "texts.csv"), "a", encoding="utf-8") as f:
        escaped = text.replace('"', '""')
        f.write(f"{doc_id},\"{escaped}\"\n")

def write_metrics_to_tempfile(temp_dir: str, doc_id: int, metrics: Dict) -> None:
    """Append metrics JSON to a temporary file."""
    with open(os.path.join(temp_dir, "metrics.csv"), "a", encoding="utf-8") as f:
        f.write(f"{doc_id},{json.dumps(metrics)}\n")

def read_texts_for_docs(temp_dir: str, doc_ids: List[int]) -> List[str]:
    """Read texts for given doc IDs from the temp file (memory‑efficient)."""
    if not doc_ids:
        return []
    doc_set = set(doc_ids)
    texts = []
    with open(os.path.join(temp_dir, "texts.csv"), "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        for row in reader:
            doc_id = int(row[0])
            if doc_id in doc_set:
                texts.append(row[1])
                if len(texts) >= len(doc_set):
                    break
    return texts

def read_metrics_for_doc(temp_dir: str, doc_id: int) -> Dict:
    """Read metrics for a specific doc ID (linear scan, but called only for top concepts)."""
    with open(os.path.join(temp_dir, "metrics.csv"), "r", encoding="utf-8") as f:
        for line in f:
            parts = line.strip().split(",", 1)
            if int(parts[0]) == doc_id:
...
def reset_batch_state(clear_analysis: bool = False) -> None:
    ...
    if bs and "temp_dir" in bs:
        import shutil
        try:
            shutil.rmtree(bs["temp_dir"])
        except Exception:
            pass
...
def render_batch_processing_controls() -> None:
    ...
    if bs:
        ...
        st.caption(
            f"Batch {done}/{total} • "
            f"{bs.get('docs_processed', 0)} docs processed"
        )
...
# Main batch processing function (v6.3.0)
def run_batch_analysis(
    ...
    if bs is not None and (
        bs.get("data_hash") != data_hash or bs.get("batch_size") != batch_size
    ):
    ...
    if bs is None:
        temp_dir = tempfile.mkdtemp(prefix="hea_batch_")
        with open(os.path.join(temp_dir, "texts.csv"), "w", encoding="utf-8") as f:
            f.write("doc_id,text\n")
        with open(os.path.join(temp_dir, "metrics.csv"), "w", encoding="utf-8") as f:
            f.write("doc_id,metrics\n")
        bs = {
            "data_hash": data_hash,
            ...
            "concept_texts": defaultdict(list),  # max 30 texts per concept
            "merged_graph": None,
            "extractor": None,
            "resolver": None,
            "builder": None,
    ...
    def _prune_graph_and_maps():
        nonlocal bs
        if bs["merged_graph"] is None:
            return
        freq_dict = bs["concept_freq"]
        all_nodes = list(bs["merged_graph"].nodes())
        sorted_nodes = sorted(all_nodes, key=lambda n: freq_dict.get(n, 0), reverse=True)
        keep_nodes = set(sorted_nodes[:keep_k])
        to_remove = [n for n in all_nodes if n not in keep_nodes]
        if to_remove:
            bs["merged_graph"].remove_nodes_from(to_remove)
            for n in to_remove:
                if n in bs["concept_abstract_map"]:
                    del bs["concept_abstract_map"][n]
                if n in bs["concept_texts"]:
                    del bs["concept_texts"][n]
        for n in list(bs["concept_freq"].keys()):
            if n not in keep_nodes:
                del bs["concept_freq"][n]
        gc.collect()
    ...
    def _process_one_batch(batch_num: int) -> None:
        ...
        for local_i, (_, row) in enumerate(batch_df.iterrows()):
            ...
            for c in unique_concepts:
                bs["concept_freq"][c] += 1
                batch_doc_freq[c] += 1
                bs["concept_abstract_map"][c].append(doc_id)
                if len(bs["concept_texts"][c]) < 30:
                    bs["concept_texts"][c].append((doc_id, text))
            bs["docs_processed"] += 1
            if (local_i + 1) % 50 == 0:
                gc.collect(0)
                if torch.cuda.is_available():
        ...
        batch_valid = [c for c in batch_unique if bs["concept_freq"][c] >= min_freq]
        ...
        del batch_concepts, batch_doc_freq, batch_graph, batch_df
    ...
    def _finalize() -> None:
        ...
        freq_dict = bs["concept_freq"]
        valid_concepts = sorted(
            [c for c in freq_dict if freq_dict[c] >= min_freq],
            key=lambda c: freq_dict[c], reverse=True
        )[:top_n]
        if len(valid_concepts) < 5:
        ...
        for n in drop_nodes:
            ...
            if n in bs["concept_texts"]:
                del bs["concept_texts"][n]
        concept_abstract_map = {c: bs["concept_abstract_map"].get(c, []) for c in valid_concepts}
        concept_to_id = {c: i for i, c in enumerate(valid_concepts)}
        id_to_concept = {i: c for i, c in enumerate(valid_concepts)}
        ...
        temp_dir = bs["temp_dir"]
        for concept in valid_concepts:
            doc_ids = concept_abstract_map.get(concept, [])
            values = []
            sample_ids = doc_ids[:200]
            for doc_id in sample_ids:
                metrics = read_metrics_for_doc(temp_dir, doc_id)
        ...
        all_texts_dict = {}
        for c in valid_concepts:
            for doc_id, text in bs["concept_texts"].get(c, []):
                all_texts_dict[doc_id] = text
        distill_df = compute_concept_distillation(
            valid_concepts, concept_abstract_map, all_texts_dict,
        ...
        bs["concept_freq"] = {}
        bs["concept_abstract_map"] = {}
        bs["concept_texts"] = {}
        bs["merged_graph"] = None
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        bs["done"] = True
    ...
    try:
        ...
        if bs["next_batch"] >= total_batches:
            ...
            status.update(
                label=f"Batch analysis complete! ({total_time:.1f}s, peak RSS ≈ {get_memory_usage_mb():.0f} MB)",
                state="complete", expanded=False,
            )
            st.success(
                f"✅ All {total_batches} batches processed in {total_time:.1f}s — peak memory ≈ {get_memory_usage_mb():.0f} MB"
            )
        else:
            status.update(
                label=f"Batch {bs['next_batch']}/{total_batches} complete",
                state="complete", expanded=False,
            )
            st.info(
                f"📦 {total_batches - bs['next_batch']} batch(es) remaining — click ▶️ Next batch or ⏩ All remaining in the sidebar."
            )
    except Exception as e:
...
def main() -> None:
    st.title(
        "🔬 HEA-Laser-ConceptGraph v6.3.0: Permanent OOM Fix"
    )
    st.caption(
        "Multi-level reasoning concept graph for CoCrFeNi laser AM | "
        "Memory-Safe Batch Processing (≤1 GB) | Disk Offload | "
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            with st.expander("Keyword Burst Detection", expanded=True):
                ...
                if burst_df is None:
                    with st.spinner("Computing burst detection (first time)..."):
                        burst_df = detect_keyword_bursts(
                            df_filtered, valid_concepts,
                            concept_abstract_map, selected_text_cols,
                        )
            ...
            with st.expander("Semantic Drift Detection"):
                ...
                if drift_df is None:
                    with st.spinner("Computing semantic drift (first time)..."):
                        drift_df = detect_semantic_drift(
                            df_filtered, valid_concepts,
                            concept_abstract_map, selected_text_cols,
                        )
            ...
            with st.expander("Concept Genealogy"):
                ...
                if genealogy_df is None:
                    with st.spinner("Computing genealogy (first time)..."):
                        genealogy_df = build_concept_genealogy(
                            nx_graph, valid_concepts, concept_abstract_map,
                        )
                        st.session_state.genealogy_df = genealogy_df
            ...
            with st.expander("Cross-Domain Bridge Detection"):
                ...
                if bridge_df is None:
                    with st.spinner("Computing bridge detection (first time)..."):
                        bridge_df = detect_cross_domain_bridges(
                            nx_graph, valid_concepts, concept_abstract_map,
                        )
                        st.session_state.bridge_df = bridge_df
            ...
            with st.expander("Network Motif Analysis"):
                motifs = st.session_state.get('motifs')
                if motifs is None:
                    with st.spinner("Computing network motifs (first time)..."):
                        motifs = analyze_network_motifs(nx_graph)
                        st.session_state.motifs = motifs
                if motifs:
                    col1, col2, col3, col4 = st.columns(4)
