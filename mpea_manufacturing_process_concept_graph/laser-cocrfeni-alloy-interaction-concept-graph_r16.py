...
def run_batch_analysis(
    ...
    # PATCH 4: Auto-reduce batch size if memory is tight
    initial_mem = get_memory_usage_mb()
    if initial_mem > 400:
        batch_size = max(200, batch_size // 2)
        st.warning(
            f"⚠️ High memory usage detected ({initial_mem:.0f} MB). "
            f"Reducing batch size to {batch_size} documents."
        )
    ...
    if bs["next_batch"] > 0:
        st.info(
            ...
            f"Graph has {bs['merged_graph'].number_of_nodes() if bs['merged_graph'] else 0} nodes."
        )
        current_mem = get_memory_usage_mb()
        if current_mem > 800 and bs["next_batch"] < total_batches * 0.5:
            st.warning(
    ...
    def _process_one_batch(batch_num: int) -> None:
        ...
        # Sequential extraction: bounded memory, no thread contention
        for local_i, (_, row) in enumerate(batch_df.iterrows()):
            ...
            if has_valid and text.strip() and len(bs["all_texts"]) < total_docs * 0.8:
        ...
        current_mem = get_memory_usage_mb()
        with status:
            st.write(f"  🧹 Memory after cleanup: {current_mem:.0f} MB")
    ...
    def _finalize() -> None:
        ...
        distill_df = compute_concept_distillation(
            ...
            max_docs_per_concept=30,  # v6.1: bounded TF-IDF corpus (≤1 GB)
        )
        burst_df = None
        drift_df = None
        genealogy_df = None
        ...
        # SAME analysis_data schema as the full pipeline → all tabs work
        analysis_data = {
    ...
    try:
        for b in pending:
            # PATCH 3: Robust exception handling per batch
            try:
                st.write(f"🔄 Starting batch {b + 1}/{total_batches}...")
                _process_one_batch(b)
                st.success(f"✅ Batch {b + 1} completed successfully")
            except Exception as batch_error:
                st.error(f"❌ Batch {b + 1} failed: {batch_error}")
                bs["next_batch"] = b + 1
