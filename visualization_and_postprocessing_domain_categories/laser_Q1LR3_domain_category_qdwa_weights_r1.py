...
QUERIES = {
    ...
    "Q1LR3": {
        "question": (
            "Q1LR3 — Analyze the sensitivity of the Gaussian heat source thermal cycle "
            "and subsequent melt pool penetration depth to variations in laser power "
            "and scan speed"
        ),
        "rationale": {
            "Thermodynamic state space & phase stability": (
                r"Phase stability is a downstream consequence of the thermal cycle; it does "
                r"not directly govern the Gaussian heat source sensitivity or melt pool "
                r"penetration depth — low relevance."
            ),
            "Multicomponent alloy chemistry & composition": (
                r"Alloy composition influences absorptivity and thermophysical properties "
                r"(e.g., thermal conductivity), but is secondary to the process parameters "
                r"in this sensitivity analysis — low relevance."
            ),
            "Laser processing parameters & thermal cycles": (
                r"This is the core of Q1LR3: laser power and scan speed directly dictate "
                r"the Gaussian heat source, thermal cycle (heating/cooling rates, peak "
                r"temperatures), and are the independent variables of the sensitivity "
                r"study — highest relevance."
            ),
            "Melt pool hydrodynamics & transport phenomena": (
                r"Melt pool penetration depth is a primary output of heat transfer and "
                r"fluid flow (Marangoni convection, buoyancy); highly relevant to the "
                r"second half of the research question."
            ),
            "Phase-field kinetics & microstructural evolution": (
                r"Microstructure evolves as a consequence of the thermal cycle, but the "
                r"question targets the thermal/fluid response (penetration depth) rather "
                r"than solid-state phase transformations — low relevance."
            ),
            "Physics-informed AI surrogate & digital twin": (
                r"Surrogates are frequently used for rapid sensitivity analysis of process "
                r"parameters, offering moderate relevance as a computational tool."
            ),
        }
    },
    "Q1TD1": {
        "question": "Q1TD1 — Gibbs tensor spectral (original)",
        "rationale": {
            "Thermodynamic state space & phase stability": "Original rationale placeholder.",
            "Multicomponent alloy chemistry & composition": "Original rationale placeholder.",
            "Laser processing parameters & thermal cycles": "Original rationale placeholder.",
            "Melt pool hydrodynamics & transport phenomena": "Original rationale placeholder.",
            "Phase-field kinetics & microstructural evolution": "Original rationale placeholder.",
            "Physics-informed AI surrogate & digital twin": "Original rationale placeholder.",
        }
    }
}
...
# --- RAW EVIDENCE SCORES (raw_k) FOR EACH QUERY ----------------------
PRESETS = {
    "Q1AL2 — KKS / partitioning focus": [8.5, 9.5, 1.5, 4.0, 9.0, 1.0],
    "Q1LR3 — Gaussian heat source / melt pool sensitivity": [2.0, 2.0, 9.5, 9.0, 2.5, 4.0],
...
def get_download_link(fig, dpi, fmt, transparent, qid):
    buf = BytesIO()
...
with st.sidebar.expander("🔢 QDWA Evidence Scoring (raw_k)", expanded=True):
    ...
    custom_vec = st.text_input(
        ...
        key="qdwa_custom",
        placeholder="e.g. 2.0, 2.0, 9.5, 9.0, 2.5, 4.0",
...
st.markdown(
    ...
    f"**{qid}** and normalized with a uniform prior."
)
...
with st.expander(f"🔍 Scoring rationale — why each raw_k ({qid})"):
    for i, cat in enumerate(domain_categories_full):
        st.markdown(f"**D{i+1} · {cat}** — $raw_k = {raw_data[i]:g}$  \n"
                    f"{current_rationale[cat]}")

with st.expander("✍️ Notation"):
    st.markdown(r"""
    - $W_k$ — normalized domain weight, $\sum_k W_k = 1$     
    - $raw_k$ — raw evidence score of domain $k$ (re-scored for the active query)
    - $\alpha$ — uniform prior (Laplace smoothing), default 0.25
    - Numerator $=\alpha + raw_k$ · Denominator $= 6\alpha + \sum_j raw_j$     
    - Left axis: raw evidence $k_{\mathrm{raw}}$ · Right axis: domain weight $W_k$     
    """)

st.subheader("Plot Data (raw_k vs W_k)")
st.dataframe(df)

fig = plot_dual_axis(df, cfg, qid)
st.pyplot(fig)

if legend_mode == "Below Plot (Separate Space)":
    render_html_legend(cfg, df)

st.markdown(get_download_link(fig, dpi, export_format, transparent_bg, qid),
            unsafe_allow_html=True)
