...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True, min_node_size=8, max_node_size=40, cmap_name="viridis", custom_labels=None, node_label_size=12, top_n_nodes=0, theme=None, physics_preset=None, show_edge_weights=False, edge_label_mode="hover", show_reasoning=False, use_abbreviated_labels=False, max_label_length=15, node_font_face="Inter, Segoe UI, Roboto, sans-serif", edge_label_size=10, edge_label_color=None, edge_label_position="middle", enable_node_highlight=True, show_definitions=True):
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        # ── ENHANCED: Abbreviation Logic with Inside-Node Positioning ──
        if use_abbreviated_labels and len(original_label) > max_label_length:
            ...
            node_shape = 'circle'  # Use circle shape instead of dot
            inside_font_size = max(8, min(int(size * 0.55), 14))
            font_dict = {'color': '#ffffff', 'size': inside_font_size, 'face': node_font_face, 'bold': True, 'vadjust': 0}
        else:
        ...
        # ── ENHANCED: Rich Tooltips with Ontology Definitions ──
        tooltip_content = f"""<div style='font-family:{node_font_face};'>
    ...
    for u, v in nx_graph.edges():
        ...
        edge_kwargs = dict(value=float(np.clip(w, 0.5, 5)), width=width, color={'color': color, 'highlight': theme['highlight_bg'], 'hover': theme['hover_bg'], 'opacity': 0.85}, smooth={'type': 'continuous', 'roundness': 0.35}, title=f"<span style='font-family:{node_font_face};'>Weight: <b>{w:.2f}</b><br>Type: {edge_type}<br>Inferred: {is_inferred}</span>", dashes=dashes)
    ...
    try:
        ...
        # ── ENHANCED: Interactive Node Highlighting & Edge Info Panel ──
            ...
            (function() {
                var checkExist = setInterval(function() {
                    if (typeof network !== 'undefined' && network.body) {
                        ...
                        // ── Node selection: gold ring + side panel ─────────────────
        ...
        # ── ENHANCED: Automated Node Label Legend ──
        if use_abbreviated_labels and label_map:
            st.markdown("---")
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=11, width=800, height=600, theme=None, branchvalues="total", show_labels=True, show_legend=True):
    ...
    for i, uid in enumerate(unique_ids):
        ...
        if show_labels:
            if d == 0:
                ...
                # Build chain of ancestor symbols
                chain = []
                current = uid
                visited_chain = set()
                ...
                # Combine the last 3 symbols in the chain (e.g., ★□◆)
                combo = "".join(chain[-3:]) if len(chain) > 3 else "".join(chain)
                if combo:
                    display_labels.append(f"{combo} {labels[i]}")
                else:
                    display_labels.append(f"{own_sym} {labels[i]}")
        else:
    ...
    # ── ENHANCED: Root gets theme color, subtle borders ──
    sunburst_colors = plot_colors.copy()
    for i in range(len(labels)):
        if depths[i] == 0:
            # Root: use a visible theme-matched color, not transparent
            sunburst_colors[i] = theme.get("plotly_paper", "#f8f9fa") if theme else "#f8f9fa"
    ...
    fig.update_layout(
        title="<b>CoCrFeNi MPEA Process-Material Response Domain Hierarchy</b><br><i>Size = concept frequency | Symbols show hierarchy depth</i>", 
    ...
    # ── ENHANCED: Symbol-to-Label Legend Grouped by Depth ──
    if show_legend:
        # Build legend entries
        legend_entries = []
        for i, uid in enumerate(unique_ids):
            if show_labels and display_labels[i]:
                legend_entries.append({
                    'symbol': display_labels[i].split()[0] if display_labels[i] else '',
                    'label': labels[i],
                    'value': values[i] if i < len(values) else 0,
                    'depth': depths[i],
                    'color': sunburst_colors[i]
                })

        if legend_entries:
            st.markdown("### 📊 Symbol-to-Label Legend")
            depth_names = {0: "Root (Domain)", 1: "Category (Parent)", 2: "Concept (Child)", 3: "Sub-Concept"}
        ...
        # Also show the general symbol legend
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.session_state.get('analysis_data') and st.session_state['analysis_data'].get('valid_concepts'):
            ...
            # ── ENHANCED: Sunburst legend controls ──
            st.session_state['sunburst_show_legend'] = st.checkbox("Show symbol legend", value=True, key="sunburst_show_legend_chk")
            st.session_state['sunburst_show_labels'] = st.checkbox("Show labels on chart", value=True, key="sunburst_show_labels_chk")
        else:
            ...
            st.session_state['sunburst_show_legend'] = True
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            with st.expander("Domain Hierarchy (Sunburst)"):
                ...
                show_legend = st.session_state.get('sunburst_show_legend', True)
                show_labels = st.session_state.get('sunburst_show_labels', True)
                render_sunburst_chart(labels, parents, values, cmap_name=cmap, theme=theme, branchvalues=bv_mode, show_labels=show_labels, show_legend=show_legend)
